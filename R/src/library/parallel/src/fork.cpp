...
static void close_non_child_fd(int fd)
{
	...
	/* should not happen */
	error(_("cannot close internal file descriptor"));
    close(fd);
}
...
SEXP mc_fork(SEXP sEstranged)
{
...
#ifdef MC_DEBUG
	Dprintf(_("parent[%d] created pipes: comm (C%d->M%d), sir (M%d->C%d)\n"),
	...
	/* should not happen */
	error(_("detected re-use of valid pipe ends\n"));
#endif
    res_i[0] = (int) pid;
    if (pid == 0) { /* child */
...
#ifdef MC_DEBUG
	Dprintf(_("child process %d started, master_fd=%d\n"), getpid(),
	        master_fd);
#endif
    } else { /* master process */
	child_info_t *ci;
...
#ifdef MC_DEBUG
	        Dprintf(_("process %d: error %s reading from fd %d\n"),
...
#ifdef MC_DEBUG
	        Dprintf(_("process %d: error %s writing to fd %d\n"),
		        getpid(), strerror(errno), fildes);
#endif
		return -1;
	    }
...
#ifdef MC_DEBUG
	    Dprintf(_("process %d: write() returned 0 writing to fd %d\n"),
	           getpid(), fildes);
#endif
	    return wbyte;
	}
...
SEXP mc_select_children(SEXP sTimeout, SEXP sWhich) 
{
    ...
    ci = children;
#ifdef MC_DEBUG
    Dprintf(_(" - read select %d children: \n"), sr);
#endif
