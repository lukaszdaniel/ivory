...
void VR_sammon(double *dd, Sint *nn, Sint *kd, double *Y, Sint *niter,
	...
	for (j = 1; j < n; j++)
		for (k = 0; k < j; k++) {
			...
			ee = d - sqrt(d1);
			if (d1 == 0)
				error(_("initial configuration has duplicates"));
			e += (ee * ee / d);
		}
	e /= tot;
	if (*trace)
		Rprintf(_("Initial stress        : %7.5f\n"), e);
	epast = eprev = e;

	/* Iterate */
	for (i = 1; i <= *niter; i++) {
		CORRECT: for (j = 0; j < n; j++) {
			for (m = 0; m < nd; m++)
				e1[m] = e2[m] = 0.0;
			for (k = 0; k < n; k++) {
				if (j == k)
...
void
VR_mds_fn(double *, double *, Sint *, double *, Sint *, double *, Sint *,
		Sint *, double *, Sint *, double *);

/*
 *  Download the data.
 */
void VR_mds_init_data(Sint *pn, Sint *pc, Sint *pr, Sint *orde, Sint *ordee,
		double *xx, double *p) {
	int i;
...
static void calc_dist(double *x) {
...
static double fminfn(int nn, double *x, void *dummy) {
	double ssq;
...
static void fmingr(int nn, double *x, double *der, void *dummy) {
	double ssq;
...
void VR_mds_dovm(double *val, Sint *maxit, Sint *trace, double *xx, double *tol) {
...
void VR_mds_fn(double *y, double *yf, Sint *pn, double *pssq, Sint *pd,
		double *x, Sint *pr, Sint *pncol, double *der, Sint *do_derivatives,
		double *p) {
...
void VR_ucv_bin(Sint *n, Sint *nb, Sfloat *d, Sint *x, Sfloat *h, Sfloat *u) {
	int i, nn = *n, nbin = *nb;
...
void VR_bcv_bin(Sint *n, Sint *nb, Sfloat *d, Sint *x, Sfloat *h, Sfloat *u) {
	int i, nn = *n, nbin = *nb;
	Sfloat delta, hh = (*h) / 4, sum, term;
	...
	*u = 1 / (2 * nn * hh * sqrt(M_PI))
			+ sum / (64 * nn * nn * hh * sqrt(M_PI));
}

void VR_phi4_bin(Sint *n, Sint *nb, Sfloat *d, Sint *x, Sfloat *h, Sfloat *u) {
	int i, nn = *n, nbin = *nb;
...
void VR_phi6_bin(Sint *n, Sint *nb, Sfloat *d, Sint *x, Sfloat *h, Sfloat *u) {
	int i, nn = *n, nbin = *nb;
	Sfloat delta, sum, term;
	...
	for (i = 0; i < nbin; i++) {
		...
		term =
				exp(-delta / 2)
						* (delta * delta * delta - 15 * delta * delta
								+ 45 * delta - 15);
		sum += term * x[i];
	}
...
void VR_den_bin(Sint *n, Sint *nb, Sfloat *d, Sfloat *x, Sint *cnt) {
...
void
lqs_fitlots(double *x, double *y, int *n, int *p, int *qn, int *lts, int *adj,
		int *sample, int *nwhich, int *ntrials, double *crit, int *sing,
		int *bestone, double *bestcoef, double *pk0, double *beta);

void
mve_fitlots(double *x, int *n, int *p, int *qn, int *mcd, int *sample,
		int *nwhich, int *ntrials, double *crit, int *sing, int *bestone);

static const R_CMethodDef CEntries[] = { { "VR_bcv_bin", (DL_FUNC) & VR_bcv_bin,
		6 }, { "VR_den_bin", (DL_FUNC) & VR_den_bin, 5 }, { "VR_mds_dovm",
		(DL_FUNC) & VR_mds_dovm, 5 },
		{ "VR_mds_fn", (DL_FUNC) & VR_mds_fn, 11 }, { "VR_mds_init_data",
				(DL_FUNC) & VR_mds_init_data, 7 }, { "VR_mds_unload", (DL_FUNC)
				& VR_mds_unload, 0 }, { "VR_phi4_bin", (DL_FUNC) & VR_phi4_bin,
				6 }, { "VR_phi6_bin", (DL_FUNC) & VR_phi6_bin, 6 }, {
				"VR_sammon", (DL_FUNC) & VR_sammon, 9 }, { "VR_ucv_bin",
				(DL_FUNC) & VR_ucv_bin, 6 }, { "lqs_fitlots", (DL_FUNC)
				& lqs_fitlots, 16 }, { "mve_fitlots", (DL_FUNC) & mve_fitlots,
				11 }, { NULL, NULL, 0 } };
