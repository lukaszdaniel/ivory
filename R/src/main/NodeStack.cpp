...
#endif
        std::vector<RObject *>::reverse_iterator rit = find(m_vector.rbegin(), m_vector.rend(), node);
...
// Foll. is inlined under NDEBUG:
    ...
    {
        std::vector<RObject *>::iterator start = m_vector.begin() + std::ptrdiff_t(m_protected_count);
