...
summary.aareg <- function(object, maxtime, test=c('aalen', 'nrisk'), scale=1,...) {
    if (!inherits(object, 'aareg')) stop(gettextf("'%s' argument is not an object of class %s", "object", dQuote("aareg")))
    ...
    times <- object$time[seq_len(ntime)]
    ...
    if (test=='aalen') {
        ...
        twt <-  object$nrisk[seq_len(ntime)]
        scale <- ntime/scale
        }
    ...
    tx <- as.matrix(twt * object$coefficient[seq_len(ntime),])
