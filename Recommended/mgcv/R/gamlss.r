...
#    }
   ...
   if (discrete) {
     ...
     for (i in seq_len(K)) for (j in i:K) { ## lp block loop
       for (l in seq_len(m)) { ## sp loop
         v <- rep(0,n);ind <- seq_len(n)
         for (q in seq_len(K)) { ## diagonal accumulation loop
...
## the first and second parameters...
  ...
  ## best we can do here is to use E only as a regularizer.
	...
	if (!is.null(offset[[1]])) yt1 <- yt1 - offset[[1]]
	  ...
	  startji <- rep(0,ncol(R))
          if (rrank<ncol(R)) {
            R <- R[seq_len(rrank),seq_len(rrank)]
	    piv <- piv[seq_len(rrank)]
          }
          startji[piv] <- backsolve(R,forwardsolve(t(R),Xty[piv]))
          start[jj[[2]]] <- startji
        } else { ## regular case
...
## the first and second parameters...
  ...
  ## Binarize each category and lm on 6*y-3 by category.
	...
	if (is.list(x)) { ## discrete case
          start <- rep(0,max(unlist(jj)))
          for (k in seq_along(jj)) { ## loop over the linear predictors
