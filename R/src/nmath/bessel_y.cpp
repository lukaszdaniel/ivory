...
double Rf_bessel_y(double x, double alpha)
{
	...
	return(((alpha - na == 0.5) ? 0 : Rf_bessel_y(x, -alpha) * cospi(alpha)) -
	       ((alpha      == na ) ? 0 : Rf_bessel_j(x, -alpha) * sinpi(alpha)));
    }
    else if (alpha > 1e7) {
...
double Rf_bessel_y_ex(double x, double alpha, double *by)
{
	...
	/* Using Abramowitz & Stegun  9.1.2
	 * this may not be quite optimal (CPU and accuracy wise) */
	return(((alpha - na == 0.5) ? 0 : Rf_bessel_y_ex(x, -alpha, by) * cospi(alpha)) -
	       ((alpha      == na ) ? 0 : Rf_bessel_j_ex(x, -alpha, by) * sinpi(alpha)));
    }
    else if (alpha > 1e7) {
	MATHLIB_WARNING(_("besselY(x, nu): nu=%g too large for 'bessel_y()' algorithm"), alpha);
	return ML_NAN;
...
/* ---------------------------------------------------------------------
  Mathematical constants
    ...
    PIM5 = 5*PI - 15
 ----------------------------------------------------------------------*/
    static constexpr double fivpi = 5*M_PI; //15.707963267948966192;
    static constexpr double pim5	= fivpi - 15;//  .70796326794896619231;

    /*----------------------------------------------------------------------
      Coefficients for Chebyshev polynomial expansion of
      1/gamma(1-x), abs(x) <= .5
      ----------------------------------------------------------------------*/
    static constexpr double ch[21] = { -6.7735241822398840964e-24,
