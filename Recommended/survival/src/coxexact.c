...
double coxd0(int d, int n, double *score, double *dmat, int dmax) {
	double *dn;
	...
	if (*dn == 0) { /* still to be computed */
		*dn = score[n - 1] * coxd0(d - 1, n - 1, score, dmat, dmax);
		if (d < n)
			*dn += coxd0(d, n - 1, score, dmat, dmax);
	}
	return (*dn);
}
double coxd1(int d, int n, double *score, double *dmat, double *d1,
	...
	if (d1[indx] == 0) { /* still to be computed */
		d1[indx] = score[n - 1] * covar[n - 1]
				* coxd0(d - 1, n - 1, score, dmat, dmax);
		if (d < n)
			d1[indx] += coxd1(d, n - 1, score, dmat, d1, covar, dmax);
		if (d > 1)
			d1[indx] += score[n - 1]
...
double coxd2(int d, int n, double *score, double *dmat, double *d1j,
		double *d1k, double *d2, double *covarj, double *covark, int dmax) {
	int indx;
	...
	if (d2[indx] == 0) { /*still to be computed */
		...
		if (d < n)
			d2[indx] += coxd2(d, n - 1, score, dmat, d1j, d1k, d2, covarj,
					covark, dmax);
		if (d > 1)
			d2[indx] += score[n - 1]
					* (coxd2(d - 1, n - 1, score, dmat, d1j, d1k, d2, covarj,
							covark, dmax)
							+ covarj[n - 1]
									* coxd1(d - 1, n - 1, score, dmat, d1k,
											covark, dmax)
							+ covark[n - 1]
									* coxd1(d - 1, n - 1, score, dmat, d1j,
											covarj, dmax));
	}
	return (d2[indx]);
}
...
SEXP coxexact(SEXP maxiter2, SEXP y2, SEXP covar2, SEXP offset2, SEXP strata2,
	...
	for (i = 0; i < nused;) {
		...
		while (time[i] == dtime) {
			...
			ndeath += status[i];
			i++;
			if (i >= nused || strata[i] > 0)
				break; /*tied deaths don't cross strata */
		}
		if (ndeath > maxdeath)
			maxdeath = ndeath;
	}
	...
	if (temp != dmemtot) { /* the subscripts will overflow */
		error(_("(number at risk) * (number tied deaths) is too large"));
	}
	...
	for (iter = 1; iter <= maxiter; iter++) {
		...
		for (i = 0; i < nused;) {
			...
			if (ndeath > 0) { /* Add to the loglik */
				...
				dmem2 = dmem0 + (nvar + 1) * dsize; /*start for the second deriv memory */
				for (j = 0; j < nvar; j++) { /* for each covariate */
					d1[j] = coxd1(ndeath, nrisk, score + sstart, dmem0,
							dmem1[j], covar[j] + sstart, maxdeath) / d0;
					if (ndeath > 3)
						R_CheckUserInterrupt();
					u[j] -= d1[j];
					for (k = 0; k <= j; k++) { /* second derivative*/
						temp = coxd2(ndeath, nrisk, score + sstart, dmem0,
								dmem1[j], dmem1[k], dmem2, covar[j] + sstart,
								covar[k] + sstart, maxdeath);
						if (ndeath > 5)
							R_CheckUserInterrupt();
						imat[k][j] += temp / d0 - d1[j] * d1[k];
