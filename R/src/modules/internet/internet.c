...
void *in_R_HTTPOpen(const char *url, const char *headers, const int cacheOK)
{
	...
	if(rc != 200) {
	    ...
	    if(!IDquiet){
		if(len > 1024*1024)
		    REprintf(ngettext("Content type '%s' length %d byte (%0.1f MB)\n",
				      "Content type '%s' length %d bytes (%0.1f MB)\n",
				      len), type ? type : "unknown", len, len/1024.0/1024.0);
		else if(len > 10240)
...
static void *in_R_FTPOpen(const char *url)
{
	...
	if(len >= 0)
	    REprintf(_("ftp data connection made, file length %d bytes\n"), len);
	else
...
/*    if(!IDquiet) {
    ...
    if(!IDquiet) {
	if(status > 1024*1024)
	    REprintf(ngettext("Content type '%s' length %d byte (%0.1f MB)\n", "Content type '%s' length %d bytes (%0.1f MB)\n", status), buf, status, status/1024.0/1024.0);
