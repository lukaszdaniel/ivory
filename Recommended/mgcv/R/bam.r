...
## once. If there are no matrix variables then k.start = 1:(ncol(k)+1) 
  ...
  for (lp in seq_len(nlp)) { ## loop over linear predictors
    smooth.spec <- if (is.null(gp$nlp)) gp$smooth.spec else gp[[lp]]$smooth.spec
    if (length(smooth.spec)>0) for (i in seq_along(smooth.spec)) nk <- nk + as.numeric(smooth.spec[[i]]$by!="NA") +
  ...
  for (lp in seq_len(nlp)) { ## loop over linear predictors
    smooth.spec <- if (is.null(gp$nlp)) gp$smooth.spec else gp[[lp]]$smooth.spec
    if (length(smooth.spec)>0) for (i in seq_along(smooth.spec)) {
...
#  if (!is.null(exclude)) warning("'exclude' argument ignored by discrete prediction at present")
  ...
  if (any(object$nsdf)||any(object$offset!=0)) { ## deal with parametric terms...
    ...
    if (length(object$nsdf)>1) {
      ...
      ps <- pstart * 0
      ii <- rep(0,0); k <- 1
      for (i in seq_along(object$nsdf)) if (object$nsdf[i]) {
    ...
    } else { ii <- seq_len(object$nsdf); pstart <- 1; ps <- 1}
  ...
  k <- 1
  kd <- dk$k
  if (any(object$nsdf>0)) for (i in seq_along(object$nsdf)) if (object$nsdf[i]>0) {
  ...
  if (type=="terms") {
    ...
    if (n.smooth) for (i in seq_len(n.smooth)) {
    ...
    if (is.null(object$family$predict)||type=="link") {
      if (nlp>1) {
        ...
        for (i in seq_len(nlp)) fit[,i] <- Xbd(Xd,object$coefficients,kd,ks,ts,dt,object$dinfo$v,object$dinfo$qc,drop=object$dinfo$drop,lt=lpid[[i]]) + offset[[i]]
      ...
      if (se==TRUE) {
        if (nlp>1) {
          ...
          for (i in seq_len(nlp)) se.fit[,i] <- diagXVXd(Xd,object$Vp,kd,ks,ts,dt,object$dinfo$v,object$dinfo$qc,drop=object$dinfo$drop,
...
{ control <- do.call("gam.control",control)
  ...
  if (is.null(G)) { ## need to set up model!
    ...
    if (discretize && length(gp$smooth.spec)==0) {
      ...
      if (!is.null(gp$nlp)) for (i in seq_len(gp$nlp)) if (length(gp[[i]]$smooth.spec)>0) ok <- FALSE
      if (ok) {
    ...
    if (discretize) { 
      ...
      if (is.null(gp$nlp)) for (i in seq_along(gp$smooth.spec)) { 
      ...
      } else for (j in seq_along(formula)) if (length(gp[[j]]$smooth.spec)>0) for (i in 1:length(gp[[j]]$smooth.spec)) {
...
#                 paraPen=paraPen,apply.by=!discretize,drop.intercept=drop.intercept,modCon=2)
    ...
    if (discretize) {
      ...
      for (j in seq_len(npt)) { ## loop over parametric terms in each formula
	...
	attr(G$Xd[[k]],"dimnames") <- NULL ## unused and storage space otherwise huge
        if (drop.intercept[j]) {
          ...
          G$Xd[[k]] <- G$Xd[[k]][,ind,drop=FALSE] ##  drop intercept
          xat$assign <- xat$assign[ind];xat$dimnames[[2]] <- xat$dimnames[[2]][ind];
          xat$dim[2] <- xat$dim[2]-1;attributes(G$Xd[[k]]) <- xat
        }
        ...
        if (ncol(G$Xd[[k]])) {
          G$kd <- cbind(seq_len(nrow(mf)),G$kd) ## add index for parametric part to index list
          dk$k.start <- c(1,dk$k.start+1) ## and adjust k.start accordingly
	  #if (nlp>1) lpid[[j]] <- k
	  lpip[[k]] <- if (nlp==1) seq_len(G$nsdf) else attr(G$nsdf,"pstart")[j] - 1 + seq_len(G$nsdf[j]) ## coefs for this term
	  if (nlp>1) for (i in seq_along(lpi)) if (any(lpip[[k]]%in%lpi[[i]])) lpid[[i]] <- c(lpid[[i]],k)
      ...
      if (length(G$smooth)>0) for (i in seq_along(G$smooth)) { ## loop over smooths
        ts[kb] <- k
        ## first deal with any by variable (as first marginal of tensor)...
        if (G$smooth[[i]]$by!="NA") {
          dt[kb] <- 1
	...
	jj <- G$smooth[[i]]$first.para:G$smooth[[i]]$last.para; lpip[[kb]] <- jj
	if (nlp>1) { ## record which lp each discrete term belongs to (can be more than one)
          for (j in seq_len(nlp)) if (any(jj %in% lpi[[j]])) lpid[[j]] <- c(lpid[[j]],kb)
        }
        kb <- kb + 1
      } ## looping over smooths
      ## put lpid indices into coefficient index order...
      if (nlp>1) {
        for (j in seq_len(nlp)) lpid[[j]] <- lpid[[j]][order(unlist(lapply(lpip[lpid[[j]]],max)))]
	G$lpid <- lpid
      }	
