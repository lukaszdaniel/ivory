...
{ q <- nrow(rS[[1]])
  ...
  k0 <- 1
  for (i in seq_len(length(rS))) { ## unpack the rS in the new space
    crs <- ncol(rS[[i]]);
    k1 <- k0 + crs * q - 1 
...
#    }
  ...
  if (ii==maxHalf) ct <- gettext("step failed", domain = "R-mgcv")
  else if (i==200) ct <- gettext("iteration limit reached", domain = "R-mgcv")
...
## Dunn & Smyth (2005) Statistics and Computing 15:267-280.
  ...
  if (p == 1) { ## It's Poisson like
    ...
    if (sum(!is.integer(y/phi))) stop("'y' argument must be an integer multiple of phi for Tweedie(p=1)")
