...
coxph <- function(formula, data, weights, subset, na.action,
    ...
    # Grab the response and id variables
    ...
    if (length(timetrans)) {
        if (multi) stop("the tt() transform is not implemented for multi-state models")
         ...
         mf <- mf[tindex,]
         istrat <- rep(seq_along(counts$nrisk), counts$nrisk)
    ...
    if (!is.null(id) && is.null(cluster) && (missing(robust) || robust)) 
        cluster <- id

    if (missing(robust)) robust <- !is.null(cluster)
    else if (robust && is.null(cluster)) {
        if (ncol(Y) ==2) cluster <- seq.int(1, nrow(mf))
