...
quantile.survfit <- function(x, probs=c(.25, .5, .75), conf.int=TRUE, 
    ...
    if (!inherits(x, "survfit")) stop(gettextf("'%s' argument is not an object of class %s", "x", dQuote("survfit")))
    ...
    if (is.null(x$strata)) {
        ...
        if (is.matrix(x$surv) && ncol(x$surv) >1) {
            ...
            if (conf.int) {
                qupper <- qlower <- qmat
                for (strat in seq_len(nstrat)) {
                    z <- x[strat,]
                    for (i in seq_len(ncol(z$surv))) {
