...
plot.aareg <- function(x, se=TRUE, maxtime, type='s', ...) {
    ...
    if (missing(maxtime)) keep <- seq_len(length(x$time))
    else		  keep <- seq_len(sum(x$time <= maxtime))
	...
	if (se) {
		...
		dd <- dim(x$dfbeta)
		keep2 <- seq_len(length(unique(x$time[keep])))
	...
	yy <- cbind(yy, yy + qnorm(0.975)*se.yy,
			 yy - qnorm(0.975)*se.yy)
	if (ncurve >1) {
	    for (i in seq_len(ncurve)) {
		j <- c(i, i+ncurve, i+2*ncurve)
		matplot(xx, yy[,j], type=type, ..., col=1, lty=c(1,2,2),
			xlab=gettext("Time"), ylab=yylab[i])
		}
	    }
	else matplot(xx, yy, type=type, ...,  col=1, lty=c(1,2,2), 
		     xlab=gettext("Time"), ylab=yylab)
	}
    else {
	matplot(xx, yy, type=type, ...,  xlab=gettext("Time"))
	}
    }
