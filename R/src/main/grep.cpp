...
static int jit_stack_size()
{
	...
	if (p)
	{
		...
		if (xdouble >= 0 && xdouble <= 1000)
			...
			warning(_("R_PCRE_JIT_STACK_MAXSIZE invalid and ignored"));
	}
	return stmax;
}
		     ...
		     Rboolean useBytes, Rboolean use_UTF8, int *next)
{
	...
	for (ib = 0, i = 0; ib <= len-plen; i++) {
	    ...
	    used = (int) Mbrtowc(nullptr,  target+ib, R_MB_CUR_MAX, &mb_st);
	    if (used <= 0) break;
	    ib += used;
	}
    } else
	...
	if (fixed_opt) {
	   ...
	   while ((ncap = pcre_exec(re_pcre, re_pe, s, ns, offset, eflag,
		  ...
		  ovector[0], ovector[1]); */
		   ...
		   char *tmp;
		   if (nns > R_INT_MAX/2) error(_("result string is too long"));
		   nns *= 2;
...
RHIDDEN SEXP do_regexpr(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	if (perl_opt && capture_count) {
	    if (n > R_INT_MAX) error(_("vector is too long"));
	    int nn = (int) n;
	    SEXP dmn;
	    PROTECT(dmn = allocVector(VECSXP, 2));
	    SET_VECTOR_ELT(dmn, 1, capture_names);
