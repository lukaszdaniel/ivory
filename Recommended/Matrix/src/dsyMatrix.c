...
double get_norm_sy(SEXP obj, const char *typstr)
{
    ...
    return F77_CALL(dlansy)(typnm, uplo_P(obj),
			    dims, REAL(R_do_slot(obj, Matrix_xSym)),
...
SEXP dsyMatrix_rcond(SEXP obj, SEXP type)
{
    ...
    F77_CALL(dsycon)(uplo_P(trf), dims,
		     REAL   (R_do_slot(trf, Matrix_xSym)), dims,
		     INTEGER(R_do_slot(trf, Matrix_permSym)),
		     &anorm, &rcond,
...
SEXP dsyMatrix_matrix_solve(SEXP a, SEXP b)
{
	...
	*bdims = INTEGER(R_do_slot(val, Matrix_DimSym)),
    ...
    F77_CALL(dsytrs)(uplo_P(trf), adims, bdims + 1,
		     REAL(R_do_slot(trf, Matrix_xSym)), adims,
		     INTEGER(R_do_slot(trf, Matrix_permSym)),
...
{
    int n = INTEGER(R_do_slot(from, Matrix_DimSym))[0];
    SEXP val = PROTECT(allocMatrix(REALSXP, n, n));

    make_d_matrix_symmetric(Memcpy(REAL(val),
				   REAL(R_do_slot(from, Matrix_xSym)), n * n),
			    from);
...
SEXP dsyMatrix_matrix_mm(SEXP a, SEXP b, SEXP rtP)
{
	...
	*bdims = INTEGER(R_do_slot(val, Matrix_DimSym)),
    ...
    double *bcp, *vx = REAL(R_do_slot(val, Matrix_xSym));
	...
	F77_CALL(dsymm)(rt ? "R" :"L", uplo_P(a), &m, &n, &one,
			REAL(R_do_slot(a, Matrix_xSym)), adims, bcp,
			&m, &zero, vx, &m FCONE FCONE);
    // add dimnames:
    int nd = rt ?
	1 : // v <- b %*% a : rownames(v) == rownames(b)  are already there
	0;  // v <- a %*% b : colnames(v) == colnames(b)  are already there
    SEXP nms = PROTECT(duplicate(VECTOR_ELT(R_do_slot(a, Matrix_DimNamesSym), nd)));
    SET_VECTOR_ELT(R_do_slot(val, Matrix_DimNamesSym), nd, nms);
...
SEXP dsyMatrix_trf(SEXP x)
{
    ...
    AZERO(vx, n * n);
    F77_CALL(dlacpy)(uplo, &n, &n, REAL(R_do_slot(x, Matrix_xSym)), &n, vx, &n FCONE);
...
SEXP matrix_trf(SEXP x, SEXP uploP)
{
    ...
    R_do_slot_assign(val, Matrix_uploSym, uploP);
    R_do_slot_assign(val, Matrix_diagSym, mkString("N"));
    R_do_slot_assign(val, Matrix_DimSym, dimP);
