...
#define N_SMALL 200
// range limit for counting sort. Should be less than R_INT_MAX
...
inline static int icheck(int x)
{
    ...
    return ((nalast != 1) ? ((x != NA_INTEGER) ? x*order : x) :
	    ((x != NA_INTEGER) ? (x*order) - 1 : R_INT_MAX));
}
   ...
   Pushes group sizes onto stack */
{
	...
	skip[radix] = (radixcounts[radix][i] == (unsigned int) n);
    ...
    if (radix_xsuballoc < (size_t) maxgrpn) {
...
static void dradix(unsigned char *x, int *o, int n)
{
	...
	for (size_t radix = 0; radix < colSize; radix++)
    ...
    if ((int) radix_xsuballoc < maxgrpn) {
...
HIDDEN SEXP do_radixsort(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
	...
	case LGLSXP:
	    ...
	    g = reinterpret_cast<void(*)()>(isort);
	    break;
	case REALSXP:
	    twiddle = &dtwiddle;
	    is_nan = &dnan;
	    f = reinterpret_cast<int(*)()>(dsorted);
	    g = reinterpret_cast<void(*)()>(dsort);
	    break;
	case STRSXP:
	    f = reinterpret_cast<int(*)()>(csorted);
	    if (sortStr) {
		csort_pre((SEXP*) xd, n);
		alloc_csort_otmp(gsmax[1 - flip]);
		g = reinterpret_cast<void(*)()>(csort);
	    }
	    // no increasing/decreasing order required if sortStr = FALSE,
	    // just a dummy argument
	    else
		g = reinterpret_cast<void(*)()>(cgroup);
	    break;
	default:
	...
	for (int grp = 0; grp < ngrp; grp++) {
		...
		if (nalast == 0) {
            ...
            tmp = reinterpret_cast<int(*)(void *, int)>(*f)(xsub, thisgrpn);
            if (tmp) {
	    ...
	    reinterpret_cast<int(*)(void *, int *, int)>(*g)(xsub, osub, thisgrpn);
