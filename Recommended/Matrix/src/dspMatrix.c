...
double get_norm_sp(SEXP obj, const char *typstr)
{
    ...
    return F77_CALL(dlansp)(typnm, uplo_P(obj), dims,
			    REAL(R_do_slot(obj, Matrix_xSym)), work FCONE FCONE);
}
...
SEXP dspMatrix_rcond(SEXP obj, SEXP type)
{
    ...
    F77_CALL(dspcon)(uplo_P(trf), dims,
		     REAL   (R_do_slot(trf, Matrix_xSym)),
...
SEXP dspMatrix_solve(SEXP a)
{
    ...
    F77_CALL(dsptri)(uplo_P(val), dims, REAL(R_do_slot(val, Matrix_xSym)),
		     INTEGER(R_do_slot(trf, Matrix_permSym)),
...
SEXP dspMatrix_matrix_solve(SEXP a, SEXP b)
{
	...
	*bdims = INTEGER(R_do_slot(val, Matrix_DimSym));
    ...
    F77_CALL(dsptrs)(uplo_P(trf),
		     &n, &nrhs, REAL(R_do_slot(trf, Matrix_xSym)),
		     INTEGER(R_do_slot(trf, Matrix_permSym)),
...
SEXP dspMatrix_matrix_mm(SEXP a, SEXP b)
{
    ...
    int *bdims = INTEGER(R_do_slot(val, Matrix_DimSym));
    ...
    double *ax = REAL(R_do_slot(a, Matrix_xSym)), one = 1., zero = 0.,
	*vx = REAL(R_do_slot(val, Matrix_xSym)), *bx;
...
SEXP dspMatrix_trf(SEXP x)
{
    ...
    F77_CALL(dsptrf)(uplo, dims, REAL(R_do_slot(val, Matrix_xSym)), perm, &info FCONE);
