...
HIDDEN R_xlen_t R::Rf_OneIndex(SEXP x, SEXP s, R_xlen_t nx, int partial, SEXP *newname,
	 int pos, SEXP call)
{
    SEXP names;
    R_xlen_t i, indx;
...
HIDDEN R_xlen_t R::Rf_get1index(SEXP s, SEXP names, R_xlen_t len, int pok, int pos, SEXP call)
{
 ...
 */
HIDDEN SEXP R::Rf_vectorIndex(SEXP x, SEXP thesub, int start, int stop, int pok, SEXP call,
	    bool dup)
{
    int i;
    R_xlen_t offset;
...
HIDDEN SEXP R::Rf_mat2indsub(SEXP dims, SEXP s, SEXP call)
{
    int nrs = nrows(s);
    R_xlen_t NR = nrs;
    SEXP rvec;
...
HIDDEN SEXP R::Rf_strmat2intmat(SEXP s, SEXP dnamelist, SEXP call)
{
...
static SEXP realSubscript(SEXP s, R_xlen_t ns, R_xlen_t nx, R_xlen_t &stretch, SEXP call)
{
	...
	for (i = 0; i < ns; i++) {
	    ...
	    if (!R_FINITE(ds)) {
		if (ds > R_INT_MAX) int_ok = FALSE;
		cnt++;
	    } else if ((R_xlen_t) ds != 0) cnt++;
#else
	    if (R_FINITE(ds) && ds > R_INT_MAX) int_ok = FALSE;
...
HIDDEN SEXP R::Rf_makeSubscript(SEXP x, SEXP s, R_xlen_t &stretch, SEXP call)
{
