...
survreg.fit<- function(x, y, weights, offset, init, controlvals, dist, 
    ...
    if (nstrat>1 && (missing(strata) || length(strata)!= n))
	    stop(gettextf("invalid '%s' argument", "strata"))
	...
	wt <-  -1*deriv$ddg*weights
	coef <- coxph.wtest(t(x)%*% (wt*x), c((wt*eta + weights*deriv$dg)%*% x), toler.chol=toler.chol)$solve
	init <- c(coef, vars)
	}
    ...
    cname <- dimnames(x)[[2]]
    if (is.null(cname)) cname <- paste("x", seq_len(ncol(x)))
    if (scale==0) cname <- c(cname, rep("Log(scale)", nstrat))
    if (scale>0) fit$coef <- fit$coef[seq_len(nvar2)]
    ...
    temp <- list(coefficients   = coef.new,
		 ...
		 loglik = loglik, 
		 iter   = fit$iter,
		 linear.predictors = c(x %*% fit$coef[seq_len(nvar)] + offset),
                 df= length(fit$coef),
		 score = fit$u
		 )
    temp
