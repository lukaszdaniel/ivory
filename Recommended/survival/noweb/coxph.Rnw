...
\begin{enumerate}
  \item Gather all the variables that appear on a right-hand side, and
    create a master formula y ~ all of them.  This is use to create the
...
There may still be rows that are missing \emph{some} of the covariates but
...
the current version was created before removing missings, and can
...
In t2 the column labels are x1:x3 and x3:x2 --- note left-right swap of 
