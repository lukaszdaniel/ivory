...
tmerge <- function(data1, data2, id, ..., tstart, tstop, options) {
    ...
    tmerge.control <- function(id="id", tstart="tstart", tstop="tstop",  defer =0) {
        if (length(defer) !=1 || !is.numeric(defer) || defer <0)
            stop("'defer' argument must be a non-negative number")
        if (!is.character(id)) stop(gettextf("'%s' argument must be a character string", "id"))
        if (!is.character(tstart)) stop(gettextf("'%s' argument must be a character string", "tstart"))
         if (!is.character(tstop)) stop(gettextf("'%s' argument must be a character string", "tstop"))
    ...
    for (ii in seq(along.with=args)) {
        ...
        if (length(etime) != length(id)) 
            stop(gettextf("argument %s is not the same length as id", argname[ii]))
        ...
        if (length(argi) >1) {
            if (length(argi) > 2) 
                stop(gettextf("too many variables in an %s call", paste(argclass[ii], collapse = ", ")))
            if (diff(sapply(argi, length)) !=0)
                stop(gettextf("different lengths in an %s call", paste(argclass[ii], collapse = ", ")))
            if (!is.numeric(yinc) && argclass[ii] != "event") 
                stop(gettextf("non numeric increment in an %s call", paste(argclass[ii], collapse = ", ")))
