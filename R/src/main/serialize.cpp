...
/* Rsnprintf: like snprintf, but guaranteed to null-terminate. See
   errors.cpp::Rvsnprintf_mbcs for a multi-byte safe version. */
...
static int PackFlags(int type, int levs, int isobj, int hasattr, int hastag)
{
    ...
    */
    if (type == CHARSXP) levs = CXXR::String::clearCacheAndHashMasks(levs);
...
{
    *ptype = SEXPTYPE(DECODE_TYPE(flags));
...
#define MAX_PACKED_INDEX (R_INT_MAX >> 8)
...
namespace R
{
	RHIDDEN int R_ReadItemDepth = 0, R_InitReadItemDepth;
}
...
extern "C"
RHIDDEN SEXP do_serializeToConn(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
/* unserializeFromConn(conn, hook) used from readRDS().
   ...
   connections internally */
extern "C"
RHIDDEN SEXP do_unserializeFromConn(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
static void OutBytesMem(R_outpstream_t stream, /*const*/ void *buf, int length)
{
    ...
    if((double) mb->count + length > (double) R_INT_MAX)
	error(_("serialization is too large to store in a raw vector"));
#endif
...
RHIDDEN SEXP R_unserialize(SEXP icon, SEXP fun)
{
    struct R_inpstream_st in;
    SEXP (*hook)(SEXP, SEXP);
...
extern "C"
RHIDDEN SEXP do_lazyLoadDBflush(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
extern "C"
RHIDDEN SEXP do_lazyLoadDBfetch(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
extern "C"
RHIDDEN SEXP do_getVarsFromFrame(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
extern "C"
RHIDDEN SEXP do_lazyLoadDBinsertValue(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
extern "C"
RHIDDEN SEXP do_serialize(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
