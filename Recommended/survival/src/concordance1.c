	...
	for (i = n - 1; i >= 0;) {
		...
		if (status[i] == 1) { /* process all tied deaths at this point */
			for (j = i; j >= 0 && status[j] == 1 && time[j] == time[i]; j--) {
				...
				index = indx[j];
				for (k = i; k > j; k--)
					count[3] += wt[j] * wt[k]; /* tied on time */
				...
				while (index > 0) { /* walk up the tree  */
					...
					if (index & 1) /* I am the left child */
						...
						count[0] += wt[j] * (twt[parent] - twt[index]);
					index = parent;
				}
			}
		} else
		...
		for (; i > j; i--) {
			...
			vss += wsum1 * (newmean + oldmean - 2 * lmean)
					* (newmean - oldmean);
			vss += wsum3 * (newmean + oldmean + wt[i] - 2 * umean)
					* (oldmean - newmean);
	...
	for (i = 0; i < n;) {
		...
		if (status[iptr] == 1) {
			...
			for (; istart < n && time1[sort1[istart]] >= dtime; istart++) {
				...
				vss += wsum3 * (newmean + oldmean - 2 * umean)
						* (newmean - oldmean);
			...
			/* Process deaths */
			for (j = i;
					j < n && status[sort2[j]] == 1 && time2[sort2[j]] == dtime;
					j++) {
				jptr = sort2[j];
				ndeath += wt[jptr];
				index = indx[jptr];
				for (k = i; k < j; k++)
					count[3] += wt[jptr] * wt[sort2[k]];
				...
				while (index > 0) { /* walk up the tree  */
					...
					if (index & 1) /* I am the left child */
						...
						count[0] += wt[jptr] * (twt[parent] - twt[index]);
					index = parent;
				}
			}
		} else
		...
		for (; i < j; i++) {
			...
			vss += wsum1 * (newmean + oldmean - 2 * lmean)
					* (newmean - oldmean);
			vss += wsum3 * (newmean + oldmean + wt[iptr] - 2 * umean)
					* (oldmean - newmean);
