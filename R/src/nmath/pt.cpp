 ...
 */
    ...
    if(!R_FINITE(n))
	return Rf_pnorm(x, 0.0, 1.0, lower_tail, log_p);

#ifdef R_version_le_260
    if (n > 4e5) { /*-- Fixme(?): test should depend on `n' AND `x' ! */
	/* Approx. from	 Abramowitz & Stegun 26.7.8 (p.949) */
	val = 1./(4.*n);
	return Rf_pnorm(x*(1. - val)/sqrt(1. + x*x*2.*val), 0.0, 1.0,
		     lower_tail, log_p);
    }
#endif
     ...
     * now that Rf_pbeta(*, log_p = TRUE) is much better.
	...
	/* Danger of underflow. So use Abramowitz & Stegun 26.5.4
	   Rf_pbeta(z, a, b) ~ z^a(1-z)^b / aB(a,b) ~ z^a / aB(a,b),
	   with z = 1/nx,  a = n/2,  b= 1/2 :
	*/
	double lval;
	lval = -0.5*n*(2*log(fabs(x)) - log(n))
		- Rf_lbeta(0.5*n, 0.5) - log(0.5*n);
	val = log_p ? lval : exp(lval);
    } else {
	val = (n > x * x)
	    ? Rf_pbeta(x * x / (n + x * x), 0.5, n / 2., /*lower_tail*/0, log_p)
	    : Rf_pbeta(1. / nx,             n / 2., 0.5, /*lower_tail*/1, log_p);
    }
	...
	else return val - M_LN2; /* = log(.5* Rf_pbeta(....)) */
    }
    else {
	val /= 2.;
	return R_D_Cval(val);
