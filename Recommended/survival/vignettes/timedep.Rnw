...
status) are recorded at the end of the day.
...
\begin{itemize}
  ...
  \item Each additional call then adds either an endpoint or a covariate,
    ...
    a \code{tdc} or \code{cumtdc} one adds a time dependent covariate.
...
\begin{enumerate}
  ...
  \item A subject transplanted on day 10 is considered to have been on medical
    ...
    moving this transplant back .5 day.
...
for the event code or time dependent covariate.
If this second argument is not present a value of 1 is used.

If a created variable is not already in data1, 
the starting value \emph{before} the first definition of that
variable is a NA for a
\code{tdc} or \code{cumtdc} call that has two arguments and 0
in all other cases.
If the variable being created is already a part of data1, 
...
example above, but quite surprising results can occur when you
think a new variable is being created de novo but its name is already
in use.  For example, if we name the new variable
`transplant' in the third line of \code{sdata} above it collides
with an existing variable in the \code{jasa} data set; the result
is to not create a time-dependent transplant variable at all.
(The author made this mistake himself when creating this vignette,
 and then spent several hours searching the tmerge
code for an error that wasn't there.)
...
This is easily rectified by creating a new data set with time intervals.
...
@ 
In the new data set the \code{tgroup} variable correctly tracks time
intervals. 
...
At 30 days the Cox model curves essentially become parallel.
...
equivalent to the \code{I()} function, i.e., you cannot simply write
...
A reasonable strategy is to use \code{tt()} expressions for initial analysis
and then expanded data sets to follow up on selected models.
<<expand>>=
...
The time-transform is a newer addition and still has some rough edges.
