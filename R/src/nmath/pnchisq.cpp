...
(cd `R-devel RHOME`/src/nmath; g++ -I. -I../../src/include -I../../../R/src/include -I/usr/local/include -DHAVE_CONFIG_H -fopenmp -g -O0 -pedantic -Wall -DDEBUG_pnch -DDEBUG_q -c ../../../R/src/nmath/pnchisq.cpp -o pnchisq.o )
...
double Rf_pnchisq(double x, double df, double ncp, int lower_tail, int log_p)
{
    ...
    ans = Rf_pnchisq_raw(x, df, ncp, 1e-12, 8*DBL_EPSILON, 1000000, (Rboolean) (lower_tail), (Rboolean) (log_p));
...
#endif
	ans = Rf_pnchisq_raw(x, df, ncp, 1e-12, 8*DBL_EPSILON, 1000000, (Rboolean) (!lower_tail), FALSE);
	return log1p(-ans);
    }
}
