...
all.equal.environment <- function (target, current, all.names=TRUE, evaluate=TRUE, ...) {
    ...
    if(evaluate) {
        ...
        if(!identical(nt <- sort(names(target )),
            ...
            paste(gettext("names of environments differ:", domain = "R-base"), all.equal(nt, nc, ...), collapse=" ")
        else
            gettext("environments contain objects of the same names, but are not identical", domain = "R-base")
    }
}
