...
#define CHECK_INTEGER_OVERFLOW(call, ans, naflag)        \
	do                                                   \
	{                                                    \
		if (naflag)                                      \
		{                                                \
...
RHIDDEN SEXP do_arith(SEXP call, SEXP op, SEXP args, SEXP env)
{
    SEXP ans, arg1, arg2;
    int argc;
...
#define COERCE_IF_NEEDED(v, tp, vpi)                   \
	do                                                 \
	{                                                  \
		if (TYPEOF(v) != (tp))                         \
		{                                              \
...
#define FIXUP_NULL_AND_CHECK_TYPES(v, vpi)                                        \
	do                                                                            \
	{                                                                             \
		switch (TYPEOF(v))                                                        \
		{                                                                         \
		case NILSXP:                                                              \
			REPROTECT(v = allocVector(INTSXP, 0), vpi);                           \
			break;                                                                \
		case CPLXSXP:                                                             \
		case REALSXP:                                                             \
		case INTSXP:                                                              \
		case LGLSXP:                                                              \
			break;                                                                \
		default:                                                                  \
			errorcall(call, _("non-numeric argument passed to binary operator")); \
		}                                                                         \
	} while (0)

RHIDDEN SEXP R_binary(SEXP call, SEXP op, SEXP x, SEXP y)
{
...
RHIDDEN SEXP R_unary(SEXP call, SEXP op, SEXP s1)
{
...
static SEXP math1(SEXP sa, double(*f)(double), SEXP lcall)
{
    ...
    if (!isNumeric(sa))
	errorcall(lcall, _("non-numeric argument passed to mathematical function"));
    ...
    if(naflag) warningcall(lcall, _("NaN values produced"));
...
RHIDDEN SEXP do_math1(SEXP call, SEXP op, SEXP args, SEXP env)
{
    SEXP s;
...
RHIDDEN SEXP do_trunc(SEXP call, SEXP op, SEXP args, SEXP env)
{
	SEXP s;
	if (DispatchGroup("Math", call, op, args, env, &s))
		return s;
...
RHIDDEN SEXP do_abs(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	errorcall(call, _("non-numeric argument passed to mathematical function"));
		   ...
		   SEXP lcall)
{
	...
	constexpr double besselJY_max_nu = 1e7;
...
RHIDDEN SEXP do_math2(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
/* log{2,10}() builtins : */
RHIDDEN SEXP do_log1arg(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
RHIDDEN SEXP do_log(SEXP call, SEXP op, SEXP args, SEXP env)
{
    args = evalListKeepMissing(args, env);
    return  do_log_builtin(call, op, args, env);
}

RHIDDEN SEXP do_log_builtin(SEXP call, SEXP op, SEXP args, SEXP env)
{
    PROTECT(args);
    int n = length(args);
    SEXP res;
...
#define SETUP_Math3                                                       \
	if (!isNumeric(sa) || !isNumeric(sb) || !isNumeric(sc))               \
		error(_("non-numeric argument passed to mathematical function")); \
                                                                          \
	na = XLENGTH(sa);                                                     \
	nb = XLENGTH(sb);                                                     \
	nc = XLENGTH(sc);                                                     \
...
RHIDDEN SEXP do_math3(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
RHIDDEN SEXP do_math4(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
RHIDDEN SEXP do_math5(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
    lcall = call;
