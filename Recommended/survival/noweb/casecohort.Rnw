...
if (!is.null(attr(Terms, "specials")$id)) {
    if (!is.null(newterm$id)) 
        stop(gettextf("cannot have both an '%s' argument and a '%s' term in the formula", "id", "cluster"))
...
if (!is.null(attr(Terms, "specials")$strata)) {
    if (!is.null(newterms$stratum))
        stop(gettextf("cannot have both an '%s' argument and a '%s' term in the formula", "stratum", "strata"))
...
if (!all(subcoh %in% 0:1))
    stop("'subcoh' argument must be 0/1 or FALSE/TRUE")
...
if (!is.null(strata)) {
    ...
    if (length(cohort.size)!=length(levels(strata)))
            stop("'cohort.size' and 'stratum' arguments do not match")
...
if(!inherits(Y, "Surv"))
    stop(gettextf("'%s' argument is not an object of class %s", "formula", dQuote("Surv")))
ytype <- attr(Y, "type")
...
@ 
                 ...
                 Y[,3])
    if (any(!subcoh & !cens))
        stop(sprintf(ngettext(sum(!subcoh & !cens), "%d censored observation not in subcohort", "%d censored observations not in subcohort", domain = "R-survival"), sum(!subcoh & !cens)), domain = NA)
    cc<-cens+1-subcoh
