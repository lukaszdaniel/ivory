...
function(package, dir, lib.loc = NULL)
{
    ...
    if(!missing(package)) {
        ...
        if(packageHasNamespace(package, dirname(dir))) {
            ...
            auto_namespace <- grepl("# Default NAMESPACE created by R", ns[1L], useBytes = TRUE)
...
function()
{
    ...
    c(if(identical(as.logical(Sys.getenv("_R_CHECK_RD_USAGE_METHOD_SUBSET_")), FALSE))
      c("[", "[[", "$", "[<-", "[[<-", "$<-"),
      if(identical(as.logical(Sys.getenv("_R_CHECK_RD_USAGE_METHOD_BINOPS_")), FALSE))
      c("+", "-", "*", "/", "^", "<", ">", "<=", ">=", "!=", "==", "%%", "%/%", "&", "|"), "!")
}
## </FIXME>
