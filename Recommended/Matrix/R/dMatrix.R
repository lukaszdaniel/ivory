...
setAs("dMatrix", "nMatrix",
	  ...
	  if(anyNA(from@x) && ((.w <- isTRUE(getOption("Matrix.warn"))) ||
	      ...
	      if(.w) warning(gettextf("object of class %s with NA values coerced to an object of class %s:  NA |-> TRUE", dQuote("dMatrix"), dQuote("nMatrix")))
	      else message(gettextf("object of class %s with NA values coerced to an object of class %s:  NA |-> TRUE", dQuote("dMatrix"), dQuote("nMatrix")))
