...
These can be detected when catching up the risk set at a new death time:
\emph{both} the start and stop time of a too-be-added obs will be larger than
the current death time.  
Since time intervals are $>0$ this will never happen for a death, since by
definition their time will match the current dtime.
Mark those who were valid, so that at a later step we don't take out someone
who was never put in.
...
chsolve2(imat,nvar,u);        /* u replaced by  u *inverse(i) */
*sctest=0;
for (i=0; i<nvar; i++) {
    *sctest +=  u[i]*a[i];
}
...
If iter=0 the loop below will never execute.  The pattern is to
1. ask: did the last update work? and update beta-hat accordingly, then
2.  compute the statistics on this new beta-hat
<<agfit4-iter>>=
/* main loop */
    ...
    if (*iter ==1) {
	    ...
	    ** The routine has not made progress past the last good value.
            */
            ...
            /* are we done? */
            if (fail ==0 && halving ==0 &&
                fabs(1-(loglik[1]/newlk)) <= eps) goto done;
...
if (maxiter>1 && newlk > loglik[1]) {
    /* 
    ** "Once more unto the breach, dear friends, once more; ..."
    ** We need to use beta and imat at the last good value, not the last 
    **  attempted value. We have tossed the old imat away, so recompute it.
    ** This will happen very rarely that we run out of iterations.
    ** If maxiter=1, we return 1 iteration whether it worked or not. 
    */
...
loglik[1] = newlk;
chinv2(imat, nvar);
(*iter)--;  /* the loop always indexes one more than we actually did */
for (i=0; i<nvar; i++) {
