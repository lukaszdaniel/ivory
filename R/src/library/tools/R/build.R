...
##		ensure_installed()
		...
		if (!all(exclude)) {
			...
			if (!all(copied)) {
			    warning(sprintf(ngettext(sum(!copied), "%s file %s\n  ignored as vignettes have been rebuilt.\n  Run R CMD build with --no-build-vignettes to prevent rebuilding.", "%s files %s\n  ignored as vignettes have been rebuilt.\n  Run R CMD build with --no-build-vignettes to prevent rebuilding.", domain = "R-tools"), sQuote("inst/doc"), strwrap(paste(sQuote(basename(tocopy[!copied])), collapse=", "),
                                            indent = 4, exdent = 2)), domain = NA, call. = FALSE)
