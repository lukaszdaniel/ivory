...
gamSim <- function(eg=1,n=400,dist="normal",scale=2,verbose=TRUE) {
  ...
  if (eg==1||eg==7) { ## 4 term Gu  and Wahba example
    if (eg==1) { if (verbose) cat(gettext("Gu & Wahba 4 term additive model", domain = "R-mgcv"), "\n", sep = "") }
    else  { if (verbose) cat(gettext("Gu & Wahba 4 term additive model + correlated predictors", domain = "R-mgcv"), "\n", sep = "")}
    x0 <- runif(n, 0, 1)
    ...
    if (dist=="normal") {
      ...
      y <- rbinom(g,1,g)
    } else stop("distribution was not recognised")
    ...
    if (verbose) cat(gettext("Bivariate smoothing example", domain = "R-mgcv"), "\n", sep = "")
    ...
    if (verbose) cat(gettext("Continuous 'by' variable example", domain = "R-mgcv"), "\n", sep = "")
    x1 <- runif(n, 0, 1)
    x2 <- sort(runif(n, 0, 1))
    ...
    if (verbose) cat(gettext("Factor 'by' variable example", domain = "R-mgcv"), "\n", sep = "")
    n <- 400   
    x0 <- runif(n, 0, 1)
    x1 <- runif(n, 0, 1)
    x2 <- runif(n, 0, 1)
    ...
    if (verbose) cat(gettext("Additive model + factor", domain = "R-mgcv"), "\n", sep = "")
    x0 <- rep(1:4,50)
    x1 <- runif(n, 0, 1)
    x2 <- runif(n, 0, 1)
    x3 <- runif(n, 0, 1)
    ...
    if (verbose) cat(gettext("Gu & Wahba 4 term additive model + random effect", domain = "R-mgcv"))
