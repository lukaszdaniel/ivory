...
{ q <- nrow(rS[[1]])
  ...
  k0 <- 1
  for (i in seq_along(rS)) { ## unpack the rS in the new space
    crs <- ncol(rS[[i]]);
    k1 <- k0 + crs * q - 1 
 ...
 #     }
    ...
    if (EMPTY) {
        ...
        ## the deviance and trA 
         ...
         var.val <- variance(mu)
         varmu <- var.val[good]
         if (any(is.na(varmu))) stop("NA values in V(mu)")
         ...
         rV=matrix(0,ncol(x),ncol(x));
         dum <- 1
         if (control$trace) cat(gettext("calling gdi...", domain = "R-mgcv"))
...
## dropped from s.p.s
  ...
  if (!is.null(L)) {
    dH1 <- dH;dH <- list()
    if (length(rho)>0) for (j in seq_along(rho)) { 
...
## X is original model matrix, L the mapping from working to full sp
  ...
  if (!is.na(object$reml.scale)&&!is.null(object$db.drho)) { ## compute sp uncertainty correction
	...
	hess <- attr(hess,"hess1")
      ...
      if (!is.null(L)) { 
        db.drho <- db.drho%*%L[seq_len(M),,drop=FALSE] 
        M <- ncol(db.drho)
      }
      ...
      rV <- (d*t(ev$vectors))[,seq_len(M)] ## root of cov matrix
...
{  if (!deriv%in%c(1,2)) stop(gettextf("'%s' argument should be 1 or 2", "deriv"))
   ...
   for (i in seq_len(ncol(fd.db))) points(b$db.drho[,i],fd.db[,i],pch=19,cex=.3,col=i)

   cat("\n\n ", gettext("The objective...", domain = "R-mgcv"), "\n", sep = "")
...
#  }
  ...
  for (i in seq_len(200)) {
   if (control$trace) {
     cat("\n", i, " ", gettextf("newton max(|grad|) = %s", max(abs(grad)), domain = "R-mgcv"), "\n", sep = "")
   }
...
#    }
  ...
  if (ii==maxHalf) { 
    ct <- gettext("step failed", domain = "R-mgcv")
    warning("Fitting terminated with step failure - check results carefully")
  } else if (i==200) { 
    ct <- gettext("iteration limit reached", domain = "R-mgcv")
...
{ zoom <- function(lo,hi) {
  ...
  if (!is.null(family$n.theta)&&family$n.theta>0) {
    ...
    spind <- if (nind>0) family$n.theta+seq_len(nind) else rep(0,0)
    rspind <- family$n.theta + seq_len(nrow(b$dVkk))
  } else {
  ...
  else if (i==max.step) ct <- gettext("iteration limit reached", domain = "R-mgcv")
  else ct <- gettext("full convergence", domain = "R-mgcv")
  ## final fit
  b <- gam.fit3(x=X, y=y, sp=L%*%lsp+lsp0,Eb=Eb,UrS=UrS,
                offset = offset,U1=U1,Mp=Mp,family = family,weights=weights,deriv=1,
...
## less overflow prone than the raw derivatives
  ...
  } else stop("link is not implemented for extended families")
...
{ if (!inherits(fam,"family")) stop(gettextf("'%s' argument is not an object of class %s", "fam", dQuote("family")))
  ...
  } else stop("link was not recognised")
...
{ if (inherits(fam,"extended.family")) return(fam)
  ...
  } else stop("family was not recognized")
...
{ if (is.null(H)) St <- matrix(0,p,p)
    ...
    if (ncol(H)!=p||nrow(H)!=p) stop("'H' argument has wrong dimension")
  }
  theta <- exp(theta)
  m <- length(theta)
  if (m>0) for (i in seq_len(m)) {
...
## Original fixed p and phi version.
  ...
  if (!is.na(rho)&&!is.na(theta)) { ## use rho and theta and get derivs w.r.t. these
    if (length(rho)>1||length(theta)>1) stop(gettextf("only scalar '%s' and '%s' arguments are allowed", "rho", "theta"))
    ...
    work.param <- FALSE 
    if (length(p)>1||length(phi)>1) stop(gettextf("only scalar '%s' and '%s' arguments are allowed", "p", "phi"))
    rho <- log(phi)
    if (p>1&&p<2) {
      if (p <= a) a <- (1+p)/2
      if (p >= b) b <- (2+p)/2
  ...
  if (p == 2) { ## It's Gamma
    if (sum(y<=0)) stop("'y' argument must be strictly positive for a Gamma density")
  ...
  if (p == 1) { ## It's Poisson like
    ...
    if (all.equal(y/phi,round(y/phi))!=TRUE) stop("'y' argument must be an integer multiple of phi for Tweedie(p=1)")
...
## Dunn & Smyth (2005) Statistics and Computing 15:267-280.
  ...
  if (all(!is.na(rho))&&all(!is.na(theta))) { ## use rho and theta and get derivs w.r.t. these
    ...
    work.param <- FALSE
    if (all.derivs) warning("'all.derivs' argument only available in rho, theta parameterization")
