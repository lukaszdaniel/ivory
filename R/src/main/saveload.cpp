...
static void RemakeNextSEXP(FILE *fp, NodeInfo &node, int version, InputRoutines &m, SaveLoadData *d)
{
	...
	break;
    case ENVSXP:
	s = GCNode::expose(new Environment());
	/* skip over CAR, CDR, and TAG */
	/* CAR(s) = */ m.InInteger(fp, d);
	/* CDR(s) = */ m.InInteger(fp, d);
	/* TAG(s) = */ m.InInteger(fp, d);
...
static SEXP NewLoadSpecialHook(int type)
{
    switch (type) {
...
static void NewDataSave(SEXP s, FILE *fp, OutputRoutines *m, SaveLoadData *d)
{
    ...
    cntxt.setContextEnd(&newdatasave_cleanup, &cinfo);
			 ...
			 InputRoutines *m, SaveLoadData *d)
{
    ...
    case PROMSXP:
    {
        GCStackRoot<Environment> env(SEXP_downcast<Environment *>(NewReadItem(sym_table, env_table, fp, m, d)));
        GCStackRoot<> val(NewReadItem(sym_table, env_table, fp, m, d));
        GCStackRoot<> valgen(NewReadItem(sym_table, env_table, fp, m, d));
        GCStackRoot<Promise> prom(new Promise(valgen, env), true);
        prom->setValue(val);
        PROTECT(s = prom);
    }
    break;
...
static SEXP NewDataLoad(FILE *fp, InputRoutines *m, SaveLoadData *d)
{
    ...
    cntxt.start(CTXT_CCODE, R_NilValue, R_BaseEnv, R_BaseEnv, R_NilValue, R_NilValue);
    cntxt.setContextEnd(&newdataload_cleanup, &cinfo);
    ...
    for (count = 0; count < env_count; ++count) {
        ...
        SET_HASHTAB(env, NewReadItem(sym_table, env_table, fp, m, d));
...
static char *InStringBinary(FILE *fp, SaveLoadData *unused)
{
    ...
    if (fread(buf, sizeof(char), nbytes, fp) != (size_t) nbytes)
...
inline static void OutRealXdr(FILE *fp, double x, SaveLoadData *d)
{
    if (!xdr_double(&d->xdrs, &x))
...
inline static void OutComplexXdr(FILE *fp, Rcomplex x, SaveLoadData *d)
{
    if (!xdr_double(&d->xdrs, &(x.r)) || !xdr_double(&d->xdrs, &(x.i)))
        error(_("an xdr complex data write error occurred"));
}

inline static Rcomplex InComplexXdr(FILE *fp, SaveLoadData *d)
{
    Rcomplex x;
    if (!xdr_double(&d->xdrs, &(x.r)) || !xdr_double(&d->xdrs, &(x.i)))
...
inline static int defaultSaveVersion()
{
    static int dflt = -1;
...
inline static void saveload_cleanup(void *data)
{
    FILE *fp = (FILE *)data;
    fclose(fp);
}
...
HIDDEN SEXP do_save(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    cntxt.start(CTXT_CCODE, R_NilValue, R_BaseEnv, R_BaseEnv, R_NilValue, R_NilValue);
    cntxt.setContextEnd(&saveload_cleanup, fp);
...
inline static SEXP R_LoadSavedData(FILE *fp, SEXP aenv)
{
