...
#define LOOP_WITH_INTERRUPT_CHECK(LOOP, ncheck, n, ...)                              \
	...
	{                                                                                \
		...
		{                                                                            \
			R_xlen_t __intr_end__ = n < __intr_threshold__ ? n : __intr_threshold__; \
...
#define ITERATE_BY_REGION_PARTIAL0(sx, px, idx, nb, etype, vtype,                             \
	...
	{                                                                                         \
		...
		{                                                                                     \
			nb = __ibr_n__ - idx > GET_REGION_BUFSIZE ? GET_REGION_BUFSIZE : __ibr_n__ - idx; \
...
#define ITERATE_BY_REGION_PARTIAL_REV0(sx, px, idx, nb, etype, vtype,                   \
	...
	{                                                                                   \
		etype __ibr_buf__[GET_REGION_BUFSIZE];                                          \
		R_xlen_t __ibr_n__ = strt + nfull;                                              \
		R_xlen_t nb = nfull > GET_REGION_BUFSIZE ? GET_REGION_BUFSIZE : nfull;          \
		for (R_xlen_t idx = __ibr_n__ - nb; nb > 0; idx -= nb)                          \
		{                                                                               \
			etype *px = (etype *)GET_REGION_PTR(sx, idx, nb,                            \
												__ibr_buf__, vtype);                    \
			expr                                                                        \
				nb = idx - strt > GET_REGION_BUFSIZE ? GET_REGION_BUFSIZE : idx - strt; \
		}                                                                               \
	} while (0)
...
#define ITERATE_BY_REGION_PARTIAL_REV(sx, px, idx, nb, etype, vtype,  \
	...
	{                                                                 \
		...
		{                                                             \
			...
			ITERATE_BY_REGION_PARTIAL_REV0(sx, px, idx, nb, etype,    \
...
#define ITERATE_BY_REGION0(sx, px, idx, nb, etype, vtype, expr)   \
	do                                                            \
	{                                                             \
		ITERATE_BY_REGION_PARTIAL0(sx, px, idx, nb, etype, vtype, \
								   0, XLENGTH(sx), expr);         \
