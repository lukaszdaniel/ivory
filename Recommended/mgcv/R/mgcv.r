...
## of column indices for each linear predictor 
  ...
  if (!is.null(sp)&&length(G$sp)>0) sp <- sp[-(seq_along(G$sp))] ## need to strip off already used sp's
  ...
  if (d>1) for (i in 2:d) {
    ...
    if (!is.null(sp)&&length(um$sp)>0) sp <- sp[-(seq_along(um$sp))] ## need to strip off already used sp's
