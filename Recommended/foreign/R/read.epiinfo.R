...
read.epiinfo <- function (file, read.deleted = FALSE,
                          ...
                          lower.case.names = FALSE)
  {
    ...
    header$start <- cumsum(c(1L, header$len))[seq_len(headerlength)]
    ...
    nvars <- NROW(header)
    data <- as.data.frame(lapply(seq_len(nvars), function(i) I(substring(datalines, header$start[i], header$stop[i]))))
