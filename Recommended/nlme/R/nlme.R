  ...
  repeat {                              ## alternating algorithm
    ...
    if (controlvals$opt == "nlminb") {
      ...
      if(optRes$convergence && controlvals$msWarnNoConv)
        warning(paste(gettextf(
          "Iteration %d, LME step: nlminb() did not converge (code = %d).",
          numIter, optRes$convergence, domain = "R-nlme"),
          if(convIter >= control$iter.max) gettext("Do increase 'msMaxIter'!", domain = "R-nlme")
          else if(!is.null(msg <- optRes$message)) paste(gettext("PORT message:", domain = "R-nlme"), msg)), domain = NA)
    } else { ## nlm(.)
      ...
      if(aNlm$code > 2 && controlvals$msWarnNoConv)
        warning(paste(gettextf(
          ...
          numIter, aNlm$code, domain = "R-nlme"),
          if(aNlm$code == 4) gettext("Do increase 'msMaxIter'!", domain = "R-nlme")), domain = NA)
    }
    ...
    if (verbose) cat(gettext(" Beginning PNLS step: .. ", domain = "R-nlme"))
    work <- .C(fit_nlme,
	       ...
	       as.double(controlvals$sigma),
               nlModel,
	       NAOK = TRUE)
    if (verbose) cat(gettext(" completed fit_nlme() step.\n", domain = "R-nlme"))
	...
	controlvals$maxIter, domain = "R-nlme")
      if (controlvals$returnObject) {
	warning("maximum number of iterations reached without convergence"); break
      } else
