...
void singleXj(double *Xj, double *X, int *m, int *k, int *n, int *j) {
	...
	for (pe = Xj + *n; Xj < pe; Xj++, k++)
		*Xj = X[*k];
} /* singleXj */

void tensorXj(double *Xj, double *X, int *m, int *p, int *dt, int *k, int *n,
		int *j) {
	...
	for (i = 0; i < *dt; i++) {
		...
		for (p0 = Xj; p0 < p1; p0++, k++)
			*p0 *= M[*k];
		X += m[i] * p[i]; /* move to the next marginal matrix */
	}
} /* tensorXj */

void singleXty(double *Xy, double *temp, double *y, double *X, int *m, int *p,
		int *k, int *n) {
...
void tensorXty(double *Xy, double *work, double *work1, double *y, double *X,
	...
	for (i = 0; i < pb; i++) {
		...
		for (p0 = y, p1 = work; p0 < yn; p0++, p1++)
			*p1 = *p0; /* copy y to work */
		j = *dt - 1;
		tensorXj(work, X, m, p, &j, k, n, &i);
		/* now form M'work */
		singleXty(Xy + i * pd, work1, work, M, m + *dt - 1, &pd,
				k + (ptrdiff_t) *n * (*dt - 1), n);
	}
} /* tensorXty */

void singleXb(double *f, double *work, double *X, double *beta, int *k, int *m,
		int *p, int *n) {
...
void tensorXb(double *f, double *X, double *C, double *work, double *beta,
	...
	if (*qc <= 0) { /* no constraint supplied */
		F77_CALL(dgemm)(&trans, &trans, &md, &pb, &pd, &done, M, &md, beta, &pd,
				&dzero, C, &md);
...
void Xbd(double *f, double *beta, double *X, int *k, int *m, int *p, int *n,
	...
	for (q = i = 0; i < *nt; i++) { /* work through the terms */
		for (j = 0; j < dt[i]; j++, q++) { /* work through components of each term */
			...
			if (j > 0 && j == dt[i] - 1) {
				...
				if (c1 > dC)
					dC = c1; /* dimension of working matrix C */
			}
			if (j == 0)
				pt[i] = p[q];
			else
				pt[i] *= p[q]; /* term dimension */
		}
		if (qc[i] > 0)
			voff[i + 1] = voff[i] + pt[i];
		else
			voff[i + 1] = voff[i]; /* start of ith v matrix */
		if (maxp < pt[i])
			maxp = pt[i];
		if (qc[i] <= 0)
			tps[i + 1] = tps[i] + pt[i]; /* where ith terms starts in param vector */
		else
			tps[i + 1] = tps[i] + pt[i] - 1; /* there is a tensor constraint to apply - reducing param count*/
	}
	...
	for (j = 0; j < *bc; j++) {
		for (i = 0; i < *nt; i++) { /* work through terms */
			if (i == 0)
				f0 = f; /* result written straight to f for i==0 */
			if (dt[i] == 1)
				singleXb(f0, work, X + off[ts[i]], beta + tps[i],
						k + (ptrdiff_t) *n * ts[i], m + ts[i], p + ts[i], n);
			else
				tensorXb(f0, X + off[ts[i]], C, work, beta + tps[i], m + ts[i],
						p + ts[i], dt + i, k + (ptrdiff_t) *n * ts[i], n,
						v + voff[i], qc + i);
			if (i > 0) {
		...
		f += *n;
		beta += tps[*nt]; /* move on to next column */
	}
	if (dC)
		FREE(C);
	FREE(work);
...
void diagXVXt(double *diag, double *V, double *X, int *k, int *m, int *p,
		int *n, int *nx, int *ts, int *dt, int *nt, double *v, int *qc, int *pv,
		int *nthreads) {
	...
	*nthreads = 1;
	...
	if (*nthreads > 1) {
		bs = *pv / *nthreads;
		while (bs * *nthreads < *pv)
			bs++;
		while (bs * *nthreads - bs >= *pv)
			(*nthreads)--;
...
#pragma omp parallel for private(j,bsj,i,kk,p0,p1,p2,p3) num_threads(*nthreads)
	...
	for (j = 0; j < *nthreads; j++) {
		...
		for (i = 0; i < bsj; i++) { /* work through this block's columns */
			...
			if (i > 0)
				ei[j * *pv + kk - 1] = 0;
			Xbd(xv + j * *n, V + kk * *pv, X, k, m, p, n, nx, ts, dt, nt, v, qc,
					&one); /* XV[:,kk] */
			Xbd(xi + j * *n, ei + j * *pv, X, k, m, p, n, nx, ts, dt, nt, v, qc,
					&one); /* X[:,kk] inefficient, but deals with constraint*/
			p0 = xi + j * *n;
			p1 = xv + j * *n;
			p2 = dc + j * *n;
			p3 = p2 + *n;
			for (; p2 < p3; p0++, p1++, p2++)
				*p2 += *p0 * *p1; /* elementwise product of XV[:,kk] X[:,kk] */
		}
	} /* parallel loop end */
	/* sum the contributions from the different threads into diag... */
	for (p0 = diag, p1 = p0 + *n, p2 = dc; p0 < p1; p0++, p2++)
		*p0 = *p2;
	for (i = 1; i < *nthreads; i++)
		for (p0 = diag, p1 = p0 + *n; p0 < p1; p0++, p2++)
			*p0 += *p2;
	FREE(xv);
	FREE(dc);
	FREE(xi);
...
void XWyd(double *XWy, double *y, double *X, double *w, int *k, int *m, int *p,
		int *n, int *nx, int *ts, int *dt, int *nt, double *v, int *qc,
	...
	for (q = i = 0; i < *nt; i++) { /* work through the terms */
		...
		if (qc[i] > 0)
			...
			voff[i + 1] = voff[i]; /* start of ith Q matrix */
		if (maxp < pt[i])
			maxp = pt[i];
		if (qc[i] <= 0)
			tps[i + 1] = tps[i] + pt[i]; /* where ith terms starts in param vector */
	...
	for (p0 = Wy, p1 = Wy + *n, p2 = w; p0 < p1; p0++, y++, p2++)
		*p0 = *y * *p2;
	if (*ar_stop >= 0) { /* AR components present (weights are sqrt, therefore) */
		...
		for (p0 = w, p1 = w + *n, p2 = Wy; p0 < p1; p0++, p2++)
			*p2 *= *p0; /* sqrt weights again */
	}
	/* now loop through terms applying the components of X'...*/
	for (i = 0; i < *nt; i++) {
		if (dt[i] > 1) { /* it's a tensor */
			tensorXty(Xy0, work, work1, Wy, X + off[ts[i]], m + ts[i],
					p + ts[i], dt + i, k + ts[i] * (ptrdiff_t) *n, n);
			if (qc[i] > 0) { /* there is a constraint to apply Z'Xy0: form Q'Xy0 and discard first row... */
				/* Q' = I - vv' */
				for (x = 0.0, p0 = Xy0, p1 = p0 + pt[i], p2 = v + voff[i];
						p0 < p1; p0++, p2++)
					x += *p0 * *p2; /* x = v'Xy0 */
				p0 = XWy + tps[i];
				p1 = p0 + pt[i] - 1;
				...
				p3 = Xy0 + 1;
				for (; p0 < p1; p0++, p2++, p3++)
					*p0 = *p3 - x * *p2; /* (I-vv')Xy0 less first element */
			} else { /* straight copy */
				for (p0 = Xy0, p1 = p0 + pt[i], p2 = XWy + tps[i]; p0 < p1;
						p0++, p2++)
					*p2 = *p0;
			}
		} else { /* it's a singleton */
			//singleXty(double *Xy,double *temp,double *y,double *X, int *m,int *p, int *k, int *n)
			singleXty(XWy + tps[i], work1, Wy, X + off[ts[i]], m + ts[i],
					p + ts[i], k + ts[i] * (ptrdiff_t) *n, n);
		}
	}
	FREE(Wy);
	FREE(Xy0);
...
void XWXd(double *XWX, double *X, double *w, int *k, int *m, int *p, int *n,
	 ...
	 */
	int r, c, i, j, q, *pt, *pd, a, b, *tps, ptot, maxp = 0, maxm = 0, pa, pb,
			kk, dk, *start, one = 1, zero = 0;
	ptrdiff_t *off, *voff;
	double *p0, *p1, *p2, *Xi, *temp, *tempn, *xwx, *xwx0, *XiB, *tempB,
			*tempnB, *x0, *x1, x;
#ifndef SUPPORT_OPENMP
	*nthreads = 1;
#endif
	...
	for (q = i = 0; i < *nt; i++) { /* work through the terms */
		for (j = 0; j < dt[i]; j++, q++) {
			if (j == dt[i] - 1)
				pd[i] = p[q]; /* the relevant dimension for deciding product ordering */
			off[q + 1] = off[q] + p[q] * (ptrdiff_t) m[q]; /* submatrix start offsets */
			if (j == 0)
				pt[i] = p[q];
			else
		...
		if (qc[i] > 0)
			...
			voff[i + 1] = voff[i]; /* start of ith v vector */
		if (maxp < pt[i])
			maxp = pt[i];
		if (qc[i] <= 0)
			tps[i + 1] = tps[i] + pt[i]; /* where ith terms starts in param vector */
...
#pragma omp for
				...
				for (kk = 0; kk < *nthreads; kk++) {
					...
					for (i = start[kk]; i < start[kk + 1]; i++) { /* loop over columns of Xb */
						/* extract Xb[:,i]... */
						if (dt[b] > 1) { /* tensor */
							for (p0 = Xi, p1 = p0 + *n; p0 < p1; p0++)
								*p0 = 1.0; /* set Xi to 1 */
							tensorXj(Xi, X + off[ts[b]], m + ts[b], p + ts[b],
									dt + b, k + (ptrdiff_t) *n * ts[b], n, &i);
						} else { /* singleton */
							singleXj(Xi, X + off[ts[b]], m + ts[b],
									k + (ptrdiff_t) *n * ts[b], n, &i);
						}
						/* apply W to Xi - for AR models this is sqrt(W) */
						for (p0 = w, p1 = w + *n, p2 = Xi; p0 < p1; p0++, p2++)
							*p2 *= *p0;
						if (*ar_stop >= 0) { /* AR components present (weights are sqrt, therefore) */
							rwMatrix(ar_stop, ar_row, ar_weights, Xi, n, &one,
									&zero, tempn);
							rwMatrix(ar_stop, ar_row, ar_weights, Xi, n, &one,
									&one, tempn); /* transpose of transform applied */
							for (p0 = w, p1 = w + *n, p2 = Xi; p0 < p1;
									p0++, p2++)
								*p2 *= *p0; /* sqrt weights again */
						}
						/* now form Xa'WXb[:,i]... */
						if (dt[a] > 1) { /* tensor */
							tensorXty(xwx + i * pt[a], tempn, temp, Xi,
									X + off[ts[a]], m + ts[a], p + ts[a],
									dt + a, k + (ptrdiff_t) *n * ts[a], n);
						} else { /* singleton */
							singleXty(xwx + i * pt[a], temp, Xi, X + off[ts[a]],
									m + ts[a], p + ts[a],
									k + (ptrdiff_t) *n * ts[a], n);
						}
			...
			if (dt[a] > 1 && qc[a] > 0) { /* first term is a tensor with a constraint */
				...
				for (j = 0; j < pt[b]; j++) {
					for (x = 0.0, p0 = x0, p1 = x0 + pt[a], p2 = v + voff[a];
							p0 < p1; p0++, p2++)
						x += *p0 * *p2;
					for (p2 = v + voff[a] + 1, p1 = x0 + pt[a], x0++; x0 < p1;
							x1++, x0++, p2++)
						*x1 = *x0 - *p2 * x;
				}
				pa = pt[a] - 1;
			...
			if (dt[b] > 1 && qc[b] > 0) { /* second term is a tensor with a constraint */
				/* copy xwx to xwx0 */
				for (p0 = xwx, p1 = p0 + pt[b] * (ptrdiff_t) pa, p2 = xwx0;
						p0 < p1; p0++, p2++)
					*p2 = *p0;
				/* row by row form xwx(I-vv') dropping first col... */
				for (j = 0; j < pa; j++) {
					for (x = 0.0, p0 = xwx0 + j, p1 = v + voff[b], p2 = p1
							+ pt[b]; p1 < p2; p0 += pa, p1++)
						x += *p0 * *p1;
					x1 = xwx + j;
					for (p0 = xwx0 + j + pa, p1 = v + voff[b], p2 = p1 + pt[b], p1++;
							p1 < p2; x1 += pa, p1++, p0 += pa)
						*x1 = *p0 - *p1 * x;
				}
				pb = pt[b] - 1;
			} else
			...
			if (pd[r] > pd[c]) { /* xwx = Xr'WXc */
				for (i = 0; i < pa; i++)
					for (j = 0; j < pb; j++)
						XWX[i + tps[r] + (j + tps[c]) * (ptrdiff_t) ptot] =
								XWX[j + tps[c] + (i + tps[r]) * (ptrdiff_t) ptot] =
										xwx[i + pa * (ptrdiff_t) j];
			} else { /* Form xwx = Xc'WXr */
				for (i = 0; i < pa; i++)
					for (j = 0; j < pb; j++)
						XWX[i + tps[c] + (j + tps[r]) * (ptrdiff_t) ptot] =
								XWX[j + tps[r] + (i + tps[c]) * (ptrdiff_t) ptot] =
										xwx[i + pa * (ptrdiff_t) j];
			}
		} /* end of block loop */
	FREE(start);
