  ...
  ## buildVignettes
    ...
    check_top_level <- function()
    {
        ...
        if (config_val_to_logical(Sys.getenv("_R_CHECK_BASHISMS_", "FALSE"))
            ...
            if (!nzchar(Sys.which("checkbashisms"))) {
                ...
                printLog0(Log,
                          gettext("A complete check needs the 'checkbashisms' script.\n", domain = "R-tools"))
                wrapLog(gettext("See section 'Configure and cleanup' in the 'Writing R Extensions' manual.\n", domain = "R-tools"))
            } else {
                for (f in c("configure", "cleanup")) {
    ...
    run_examples <- function()
    {
        run_one_arch <- function(exfile, exout, arch = "", do_diff = TRUE)
        {
            ...
            ##       warningLog(Log,
            ##                gettext("checking a package with non-ASCII example code in an ASCII locale\n", domain = "R-tools"))
            ##       enc <- ""
            ##       any <- TRUE
            ##   }
        ...
        if (!do_examples) resultLog(Log, gettext("SKIPPED", domain = "R-tools"))
            ...
            if (file.exists(exfile)) {
                ...
                if (file.exists(cntFile)) {
                    ...
                    if (test_donttest) {
                        checkingLog(Log, "checking examples with --run-donttest ...")
                        cmd <- sprintf('tools:::.createExdotR("%s", "%s", silent = TRUE, use_gct = %s, addTiming = %s, commentDontrun = %s, commentDonttest = %s)',
                                       pkgname, pkgtopdir, use_gct, do_timings,
                                       !run_dontrun, FALSE)
                        Rout <- tempfile("Rout")
                        ...
                        if (status) {
                            errorLog(Log,
                                     gettextf("Running massageExamples to create %s failed", sQuote(exfile)))
                            printLog0(Log,
    ...
    check_dependencies <- function()
    {
        ...
        if(any(lengths(res) > 0L)) {
            ...
            if(!all(names(res) %in% allowed)) {
                ...
                warningLog(Log, gettext("Skipping vignette re-building", domain = "R-tools"))
...
##        Sys.setenv("_R_CHECK_XREFS_MIND_SUSPECT_ANCHORS_" = "TRUE")
    ...
    for (pkg in pkgs) {
        ...
        if (R_check_things_in_others) {
            checkingLog(Log, gettext("checking for new files in some other directories ...", domain = "R-tools"))
            snap2 <- snapshot()
            ff <- character()
