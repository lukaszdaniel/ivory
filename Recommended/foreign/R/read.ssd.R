                     ...
                     tmpProgLoc=tempfile(), sascmd="sas")
{
    ...
    logGuess <- function (x)
    {
        ...
        if (is.na(br))
            return(x)
        return(paste(rev(rex[seq_len(br - 1L)]), sep = "", collapse = ""))
    }
    fileExtension <- function(string)
    {
        n <- nchar(string)
        chars <- substring(string, seq_len(n), seq_len(n))
    ...
    if (!inherits(sasrun, "try-error") & sysret == 0L)
    {
        ...
        cat(gettextf("SAS failed. SAS program at %s", tmpProg, domain = "R-foreign"), "\n", sep = "")
        if(.Platform$OS.type == "unix") {
            cat(gettext("a log and other error products should be in the vicinity", domain = "R-foreign"), "\n", sep = "")
            system(paste("ls -l ", tmpProgLog))
        } else {
            cat(gettextf("The log file will be %s in the current directory", paste(basename(tmpProgLoc), ".log", sep=""), domain = "R-foreign"), "\n", sep = "")
        }
