...
SEXP coxcount1(SEXP y2, SEXP strat2) {
	...
	for (i = 0; i < n; i++) {
		...
		if (status[i] == 1) {
			dtime = time[i];
			for (j = stratastart; j < i; j++)
				*rrstatus++ = 0; /*non-deaths */
			*rrstatus++ = 1; /* this death */
			/* tied deaths */
			for (j = i + 1;
					j < n && status[j] == 1 && time[j] == dtime
							&& strata[j] == 0; j++)
				*rrstatus++ = 1;
			i = j - 1;
...
SEXP coxcount2(SEXP y2, SEXP isort1, SEXP isort2, SEXP strat2) {
	...
	for (i = 0; i < n; i++) {
		...
		if (status[iptr] == 1) {
			...
			for (j = i + 1; j < n; j++) {
				jptr = sort2[j];
				if (status[jptr] == 1 && time2[jptr] == dtime
						&& strata[jptr] == 0)
					nrisk++;
				else
					break;
	...
	for (i = 0; i < n;) {
		...
		if (status[iptr] == 1) {
			...
			*rrindex++ = iptr + 1;
			for (j = i + 1; j < n; j++) {
				jptr = sort2[j];
				if (time2[jptr] == dtime && status[jptr] == 1
						&& strata[jptr] == 0) {
					atrisk[jptr] = 1;
					*rrstatus++ = 1;
					*rrindex++ = jptr + 1;
