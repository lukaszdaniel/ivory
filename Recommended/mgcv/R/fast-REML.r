...
## Cholesky generalized inverse and log det function
  ...
  E <- t(t(E)/da)
  piv <- piv[seq_len(r)]
  b[piv,piv] <- chol2inv(R[seq_len(r),seq_len(r)])
  b <- t(t(da*b)*da)
  ldet <- sum(log(diag(R)[seq_len(r)])) - sum(log(da))
...
## beta_trans = Ti beta_original T is inverse Ti
  ...
  if (inverse) {
    if (is.matrix(X)) { ## X is a cov matrix
     ...
     for (i in seq_len(nr)) if (rp[[i]]$repara) X[rp[[i]]$ind]  <- if (is.null(rp[[i]]$Qs))
