...
SEXP survfitci(SEXP ftime2, SEXP sort12, SEXP sort22, SEXP ntime2, SEXP status2,
	...
	const char *rnames[] = { "nrisk", "nevent", "ncensor", "prev", "cumhaz",
			"var", "" };
	...
	if (sefit > 0)
		...
		PROTECT(vmat2 = allocMatrix(REALSXP, 1, 1)); /* dummy object */
	vmat = REAL(vmat2);
	...
	if (sefit > 0)
		umat = (double**) dmatrix2(nperson, nstate);
	...
	for (i = 0; i < n;) {
		...
		for (; i < n; i++) {
			...
			if (etime[j] == ctime) {
				...
				ws[oldstate] -= wt[j];
				if (status[j] > 0)
					cstate[id[j]] = status[j] - 1; /*new state */
				atrisk[id[j]] = 0;
			} else
				break;
		}
