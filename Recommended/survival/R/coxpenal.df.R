...
coxpenal.df <- function(hmat, hinv, fdiag, assign.list, ptype, nvar,
	...
	hdiag <- 1/fdiag
	list(fvar2 = (hdiag-pen1)*fdiag^2, df = sum((hdiag-pen1)*fdiag), fvar = fdiag, trH = sum(fdiag))
        }
	...
	if (length(assign.list) == 1)
		list(var2 = var, df = sum(imat * hinv.full), trH = sum(diag(hinv.full)), var = hinv.full)
	else {
	    df <- trH <- NULL
	    d2 <- diag(hinv.full)
	...
	for (i in seq_len(length(assign.list))) {
	    if (sparse == i){
