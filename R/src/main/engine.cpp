...
{
    int i, this_, savedDevice, plotok;
    SEXP theList;
...
int GEstring_to_pch(SEXP pch)
{
	 ...
	 */
	unsigned int ucs = 0;
	if (mbtoucs(&ucs, CHAR(pch), R_MB_CUR_MAX) > 0) ipch = ucs;
	else error(_("invalid multibyte char in 'pch=\"c\"'"));
...
SEXP GE_LTYget(unsigned int lty)
{
	...
	if((unsigned int) linetype[i].pattern == lty) return mkString(linetype[i].name);
