...
static SEXP seq_colon(double n1, double n2, SEXP call)
{
    ...
    Rboolean useInt = (Rboolean) ((n1 <= R_INT_MAX) && (n1 == (int) n1));
    if(useInt) {
	if(n1 <= INT_MIN || n1 > R_INT_MAX)
	    useInt = FALSE;
	else {
	    /* r := " the effective 'to' "  of  from:to */
	    double dn = (double) n;
	    r = n1 + ((n1 <= n2) ? dn-1 : -(dn-1));
	    if(r <= INT_MIN || r > R_INT_MAX) useInt = FALSE;
	}
    }
    if (useInt) {
	if (n1 <= n2)
...
RHIDDEN SEXP do_colon(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    SEXP s1, s2;
    double n1, n2;
...
RHIDDEN SEXP do_rep_int(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
...
RHIDDEN SEXP do_rep_len(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    R_xlen_t ns, na;
    SEXP a, s, len;
...
RHIDDEN SEXP do_rep(SEXP call, SEXP op, SEXP args, SEXP rho)
{
...
RHIDDEN SEXP do_seq(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    SEXP ans = R_NilValue /* -Wall */, from, to, by, len, along;
    R_xlen_t i, lout = NA_INTEGER;
    bool One = Rf_length(args) == 1; // *before* messing with args ..
    ...
    */
	...
	if(by == R_MissingArg)
...
#ifdef LONG_VECTOR_SUPPORT
	    if(n > 100 * (double) R_INT_MAX)
#else
	    if(n > (double) R_INT_MAX)
#endif
		...
		/* With the current limits on integers and FEPS
		   reduced below 1/R_INT_MAX this is the same as the
	...
	if(rfrom <= R_INT_MAX && rfrom >= INT_MIN &&
	   rto   <= R_INT_MAX && rto   >= INT_MIN &&
	   rfrom == (int)rfrom &&
	   (lout <= 1 || rto == (int)rto) &&
	   (lout <= 2 || rby == (int)rby)) {
...
RHIDDEN SEXP do_seq_along(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    SEXP ans;
    R_xlen_t len;
...
RHIDDEN SEXP do_seq_len(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    R_xlen_t len;
...
RHIDDEN SEXP do_sequence(SEXP call, SEXP op, SEXP args, SEXP rho)
{
