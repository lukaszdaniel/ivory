...
anova.survreg <- function(object, ..., test = c("Chisq", "none")) {
    ...
    heading <- c(gettext("Analysis of Deviance Table"), "\n", 
		 gettextf("%s distribution with %s link", sQuote(family.obj[1]), sQuote(family.obj[2])), "\n", 
		 paste(gettext("Response: "), as.character(formula(object))[2], "\n", sep = ""),
                 if (nrow(fit$var) == length(fit$coefficients))
        paste(gettextf("Scale fixed at %s", format(object$scale, digits = getOption("digits"))), "\n", sep = "")
                 else paste("Scale estimated", "\n", sep = ""),
