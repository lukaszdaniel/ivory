...
## test family deviance derivatives w.r.t. mu
  ...
  Dmu.fd <- (dev1-dev)/eps #LUKI
  cat(gettextf("Dmu: rdiff = %s cor = %s", range(dd$Dmu-Dmu.fd), corb(dd$Dmu,Dmu.fd), domain = "R-mgcv"), "\n", sep = "")
  if (plot) {
    pch <- 19;cex <- .4
...
{  if (!deriv%in%c(1,2)) stop(gettextf("'%s' argument should be 1 or 2", "deriv"))
   ...
   if (!is.null(b$b2)) for (i in 1:ncol(b$b2)) {
     ...
     cat(gettextf("cor d2b[%d] = %s", i, cor(b$b2[,i],fd.br2[,i]), domain = "R-mgcv"),"\n", sep = "")
   }
