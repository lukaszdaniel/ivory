...
survpenal.fit<- function(x, y, weights, offset, init, controlvals, dist, 
    ...
    else if (any(weights<=0)) stop("invalid 'weights' argument, must be >0")
    ...
    if (scale <0) stop(gettextf("invalid '%s' argument", "scale"))
    if (scale >0 && nstrat >1) 
	    stop("cannot have both a fixed scale and strata")
    if (nstrat>1 && (missing(strata) || length(strata)!= n))
	    stop(gettextf("invalid '%s' or '%s' argument", "strata", "nstrat"))
	...
	ipenal <- seq_along(pattr)[!sparse]   #index for non-sparse terms
        if (full.imat) {
    ...
    for (iterx in seq_len(outer.max)) {
	fit <- .Call(Csurvreg7,
	...
	if (nfrail >0) {
	    fcoef <- fit$coef[seq_len(nfrail)]
	    coef  <- fit$coef[nfrail + seq_len(nvar2)]
	    }
	else coef <- fit$coef[seq_len(nvar2)]
	...
	if (need.df) {
		...
		if (full.imat) {
		    ...
		    temp2[seq_len(nvar), seq_len(nvar)] <- coxlist2$second
		    }
