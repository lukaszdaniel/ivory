...
)
{
	...
	for(p in packages_in_hsearch_db) {
		...
		if(!is.null(hDB)) {
		    ...
		    message(gettextf("package %s has empty hsearch data - strangely", sQuote(p)), domain = "R-utils")
                    flush.console()
                }
	    }
	    else if(!is.null(package))
                warning(gettextf("no hsearch.rds meta data for package %s", p), domain = "R-utils")
	}
...
printhsearchInternal  <- function(x, ...)
{
    ...
    vfieldnames <- c(alias = "name", concept = "keyword", keyword = NA, name = "name", title = "title")
