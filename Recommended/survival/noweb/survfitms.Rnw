...
The second is there is interpolation of a sort: the n.event and n.censor
...
At one point the code took that approach, but it was too complex.
...
if (!censored) {
    ...
    if (is.null(fit$strata)) {
        ...
        for (i in c("n.enter", "n.censor")) {
            if (!is.null(fit[[i]]))
                fit[[i]] <- unlist(sapply(seq_len(nstrat), function(x) 
