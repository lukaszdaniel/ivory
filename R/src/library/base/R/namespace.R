                           ...
                           keep.parse.data = getOption("keep.parse.data.pkgs"))
{
	...
	if(lev > 0L) message(gettextf("- loading package %s", dQuote(package)))
        ...
        ## process imports
	if(lev > 1L) message(gettextf("-- processing imports for package %s", dQuote(package)))
        ...
        if(lev > 1L) message(gettextf("-- done processing imports for package %s", dQuote(package)))
        ...
        if (file.exists(codeFile)) {
            if(lev > 1L) message(gettextf("-- loading code for package %s", dQuote(package)))
        ...
        if (file.exists(paste0(dbbase, ".rdb"))) {
            if(lev > 1L) message(gettextf("-- loading sysdata for package %s", dQuote(package)))
            lazyLoad(dbbase, env)
	}
        ...
        if(file.exists(paste0(dbbase, ".rdb"))) {
            if(lev > 1L) message(gettextf("-- loading lazydata for package %s", dQuote(package)))
            lazyLoad(dbbase, .getNamespaceInfo(env, "lazydata"))
	}

        ## register any S3 methods
        if(lev > 1L) message(gettextf("-- registerS3methods for package %s", dQuote(package)))
        registerS3methods(nsInfo$S3methods, package, env)
        if(lev > 1L) message(gettextf("-- done registerS3methods for package %s", dQuote(package)))
        ...
        on.exit(Sys.unsetenv("_R_NS_LOAD_"), add = TRUE)
        ## run the load hook
	if(lev > 1L) message(gettextf("-- running .onLoad for package %s", dQuote(package)))
        runHook(".onLoad", env, package.lib, package)
	if(lev > 1L) message(gettextf("-- done running .onLoad for package %s", dQuote(package)))
        ...
        if(.isMethodsDispatchOn() && hasS4m && !identical(package, "methods") ) {
            if(lev > 1L || lev == -5)
                message(gettextf("-- processing S4 stuff for package %s", dQuote(package)))
...
### </note>
            ...
            if(length(allGenerics)) {
                ...
                for(i in seq_along(expMethods)) {
                    mi <- expMethods[[i]]
                    if(lev > 3L) message(gettextf("---- export method %s", sQuote(mi)))
                    if(!(mi %in% exports) &&
            ...
            if(lev > 1L || lev == -5)
                message(gettextf("-- done processing S4 stuff for package %s", dQuote(package)))
        }
	...
	if(lev > 2L) message(gettextf("--- processing exports for package %s", dQuote(package)))
        namespaceExport(ns, exports)
	if(lev > 2L) message(gettextf("--- sealing exports for package %s", dQuote(package)))
        sealNamespace(ns)
        runUserHook(package, pkgpath)
        on.exit()
	if(lev > 0L) message(gettextf("- done loading package %s", dQuote(package)))
        Sys.unsetenv("_R_NS_LOAD_")
        ns
    }
}
