...
R_INLINE static void RprintTrunc(char *buf, int truncated)
{
    if(R_WarnLength < BUFSIZE - 20 &&
      (truncated || strlen(buf) == (size_t) R_WarnLength)) {
...
HIDDEN SEXP do_gettext(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	for (RCNTXT *cptr = R_GlobalContext->nextContext();
		...
		cfn = CHAR(STRING_ELT(deparse1s(CAR(cptr->getCall())), 0));
...
SEXP R_GetTracebackOnly(int skip)
{
    ...
    for (c = R_GlobalContext, ns = skip;
	 c != NULL && c->getCallFlag() != CTXT_TOPLEVEL;
	 c = c->nextContext())
                  ...
                  const char *varname, Rboolean warnByDefault)
{
	...
	if (R_GlobalContext->getCallFun() != NULL &&
			       ...
			       SEXP (*handler)(SEXP, void *), void *hdata)
{
    ...
    if (body == NULL) error(_("must supply a body function"));
