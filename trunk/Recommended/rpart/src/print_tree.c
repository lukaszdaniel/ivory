...
printme(pNode me, int id)
{
    ...
    Rprintf("\n\n");
    Rprintf(n_("Node number %d: %d observation", "Node number %d: %d observations", me->num_obs), id, me->num_obs);
    Rprintf("\t   ");
    Rprintf(_("Complexity param= %f\n  response estimate=%f,  risk/n= %f\n"), me->complexity, *(me->response_est), me->risk / me->num_obs);

    if (me->leftson)
	Rprintf(n_("  left son=%d (%d observation)", "  left son=%d (%d observations)", (me->leftson)->num_obs), 2 * id, (me->leftson)->num_obs);
    if (me->rightson)
	Rprintf(n_(" right son=%d (%d observation)", " right son=%d (%d observations)", (me->rightson)->num_obs), 2 * id + 1, (me->rightson)->num_obs);
	...
	if (i == 0)
	    ...
	    Rprintf(", ");
	    Rprintf(n_("%d observation does not split\n", "%d observations do not split\n", i), i);
        }
    } else
	Rprintf("\n");
	...
	if (rp.numcat[j] == 0) {
	    ...
	    if (ss->csplit[0] == LEFT)
		Rprintf
		    (n_("var%d < %5g to the left, improve=%5.3f,  (%d missing)\n", "var%d < %5g to the left, improve=%5.3f,  (%d missing)\n", me->num_obs - ss->count),
	    ...
	    if (rp.numcat[j] < 7) {
		Rprintf(",\t");
		Rprintf(n_("improve=%5.3f, (%d missing)\n", "improve=%5.3f, (%d missing)\n", (me->num_obs - ss->count)),
