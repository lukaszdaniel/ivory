...
setValidity("Mnumeric",
            function(object) {
                ...
                ## else
                gettext("Not a valid 'Mnumeric' class object", domain = "R-Matrix")
		})
...
setClass("sparseVector",
	 ...
	 prototype = prototype(length = 0),
         validity = function(object) {
	     n <- object@length
	     if(anyNA(i <- object@i))	 gettext("'i' slot has NA values", domain = "R-Matrix")
	     else if(any(!is.finite(i))) gettext("'i' slot is not all finite", domain = "R-Matrix")
	     else if(any(i < 1))	 	 gettext("'i' must be >= 1", domain = "R-Matrix")
	     else if(n == 0 && length(i)) gettext("'i' must be empty when the object length is zero", domain = "R-matrix")
	     else if(any(i > n)) gettextf("'i' must be in 1:%d", n, domain = "R-Matrix")
