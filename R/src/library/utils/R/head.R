...
checkHT <- function(n, d) {
    ...
    msg <- if(len == 0 || all(is.na(n)))
        gettext("invalid 'n' -  must contain at least one non-missing element, got none.", domain = "R-utils")
    else if(is.null(d) && len > 1L)
        gettextf("invalid 'n' - must have length one when dim(x) is NULL, got %d", len, domain = "R-utils")
    else if(!is.null(d) && len > length(d))
        gettextf("invalid 'n' - length(n) must be <= length(dim(x)), got %d > %d",
                 len, length(d), domain = "R-utils")
