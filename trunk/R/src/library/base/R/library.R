...
##                            if (!eval(parse(text=paste("current", z$op, "target"))))
                                stop(gettextf("package %s %s was found, but %s %s is required by package %s", sQuote(pkg), current, z$op, target, sQuote(pkgname)),
                ...
                if (have_vers) {
                    ...
                    for(z in zs)
                        if (length(z) > 1L) {
                            ...
                            if (!do.call(z$op, list(as.numeric_version(current), target)))
##                            if (!eval(parse(text=paste("current", z$op, "target"))))
                                stop(gettextf("package %s %s is loaded, but %s %s is required by package %s",
                                              sQuote(pkg), current, z$op, target, sQuote(pkgname)),
                                     call. = FALSE, domain = "R-base")
                        }
                }
            }
