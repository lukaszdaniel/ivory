...
SEXP nz2Csparse(SEXP x, enum x_slot_kind r_kind)
{
	...
	if(ctype < 0)
	    error(_("'x' argument is not an object of class \"n.CMatrix\""));
...
{
    if (!isMatrix(x))
	error(_("'%s' argument must be (traditional R) matrix"), "x");
	...
	error(_("'%s' argument must be character string"), "cls");
	...
	nnz = max(256, max(nr,nc));/* nnz := final number of nonzero entries, yet unknown;
...
#endif
    default:
	error(_("'%s' argument must be a logical or double vector"), "x");
	break;
    }
