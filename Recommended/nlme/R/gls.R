    ...
    function(object, level = 0.95, which = c("all", "var-cov", "coef"), ...)
{
    ...
    if (which != "coef") {		# variance-covariance included
        if (is.null(aV <- object$apVar)) {	# only sigma
            ...
            attr(val[["sigma"]], "label") <- gettext("Residual standard error:", domain = "R-nlme")
        } else {
            if (is.character(aV)) {
            ...
            attr(auxVal[["sigma"]], "label") <- gettext("Residual standard error:", domain = "R-nlme")
    ...
    function(object, type = c("response", "pearson", "normalized"), ...)
{
    ...
    if (type != "response") {
        ...
        attr(val, "label") <- gettext("Standardized residuals", domain = "R-nlme")
        if (type == "normalized") {
            if (!is.null(cSt <- object$modelStruct$corStruct)) {
                ## normalize according to inv-trans factor
                val <- recalc(cSt, list(Xy = as.matrix(val)))$Xy[, 1]
                attr(val, "label") <- gettext("Normalized residuals", domain = "R-nlme")
            }
        }
    } else {
