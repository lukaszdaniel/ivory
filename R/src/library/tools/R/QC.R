...
function(x, ...)
{
    c(character(),
      ...
      if(length(bad <- x$missing_namespace_depends)) {
          c(.pretty_format2(ngettext(length(bad), "Namespace dependency missing from DESCRIPTION Imports/Depends entries:", "Namespace dependencies missing from DESCRIPTION Imports/Depends entries:"), sQuote(bad)),
              "")
      },
      ...
      if(ly <- length(x$many_imports)) {
          c(gettextf("Imports includes %d non-default packages.", ly, domain = "R-tools"),
            strwrap(gettext("Importing from so many packages makes the package vulnerable to any of them becoming unavailable. Move as many as possible to 'Suggests' and use conditionally.", domain = "R-tools")),
            "")
      },
      ...
      if(length(bad <- x[["orphaned"]])) {
          c(sprintf(ngettext(length(bad), "Requires orphaned package: %s", "Requires orphaned packages: %s", domain = "R-tools"), .pretty_format(bad)), "")
      },
      if(length(bad <- x[["orphaned1"]])) {#IVORY
          c(if(length(bad) > 1L)
      ...
      if(length(bad <- x[["orphaned2"]])) {
          c(sprintf(ngettext(length(bad), "Suggests orphaned package: %s", "Suggests orphaned packages: %s", domain = "R-tools"), .pretty_format(bad)), "")
      }
      )
}
...
function(x)
{
    c(character(),
      ...
      if(length(bad <- x[["bad_authors_at_R_field_has_persons_with_bad_ORCID_identifiers"]])) {
          c(gettext("Authors@R field gives persons with invalid ORCID identifiers:", domain = "R-tools"),
            paste0("  ", bad))
      }
      )
}
...
function(x, ...)
{
    ...
    c(character(),
      ...
      fmt(if(length(y <- x$bad_ORCID_iDs)) { #LUKI
      ...
      fmt(c(if(length(y <- x$Rd_keywords_or_concepts_with_Rd_markup))
            ...
            if(length(y <- x$Rd_URLs_which_should_use_doi))
                paste(c(gettext("Found the following URLs which should use \\doi (with the DOI name only):", domain = "R-tools"),
                        unlist(y)),
                      collapse = "\n")))
      )
}
