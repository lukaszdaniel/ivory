...
cox.zph <- function(fit, transform='km', terms=TRUE, singledf =FALSE, 
    ...
    if (!is.null(fit$coxlist2)) { # there are penalized terms
        ...
        pmat[seq_len(nvar), seq_len(nvar)] <- fit$coxlist2$second
        pmat[seq_len(nvar) + nvar, seq_len(nvar) + nvar] <- fit$coxlist2$second
        imatr <- resid$imat + pmat
    }
    else imatr <- resid$imat

    for (ii in seq_len(nterm)) {
        jj <- asgn[[ii]]
        kk <- c(seq_len(nvar), jj+nvar)
        imat <- imatr[kk, kk]
    ...
    sgrp <- rep(seq_len(nrow(used)), apply(used, 1, max))
    for (i in seq_len(nrow(used))) {
        k <- which(used[i,] > 0)
