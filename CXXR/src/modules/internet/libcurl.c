...
in_newCurlUrl(const char *description, const char * const mode, int type)
{
    ...
    new->connclass = (char *) malloc(strlen("url-libcurl") + 1);
    if (!new->connclass) {
	free(new);
	error(_("allocation of url connection failed"));
    }
    strcpy(new->connclass, "url-libcurl");
    ...
    new->read = &Curl_read;
    new->connprivate = (void *) malloc(sizeof(struct Curlconn));
    if (!new->connprivate) {
	free(new->description); free(new->connclass); free(new);
	error(_("allocation of url connection failed"));
    }
    RCurlconn ctxt = (RCurlconn) new->connprivate;
    ctxt->bufsize = 2 * CURL_MAX_WRITE_SIZE;
    ctxt->buf = malloc(ctxt->bufsize);
    if (!ctxt->buf) {
	free(new->description); free(new->connclass); free(new->connprivate);
	free(new);
	error(_("allocation of url connection failed"));
    }
    return new;
