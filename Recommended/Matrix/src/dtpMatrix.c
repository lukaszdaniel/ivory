...
SEXP dtpMatrix_validate(SEXP obj)
{
	...
	int d = INTEGER(R_do_slot(obj, Matrix_DimSym))[0];
	R_xlen_t lx = xlength(R_do_slot(obj, Matrix_xSym));
...
double get_norm(SEXP obj, const char *typstr)
{
    ...
    return F77_CALL(dlantp)(typnm, uplo_P(obj), diag_P(obj), dims,
			    REAL(R_do_slot(obj, Matrix_xSym)), work FCONE FCONE FCONE);
}
...
SEXP dtpMatrix_rcond(SEXP obj, SEXP type)
{
    ...
    F77_CALL(dtpcon)(typnm, uplo_P(obj), diag_P(obj), dims,
		     REAL(R_do_slot(obj, Matrix_xSym)), &rcond,
...
SEXP dtpMatrix_solve(SEXP a)
{
    ...
    F77_CALL(dtptri)(uplo_P(val), diag_P(val), Dim,
		     REAL(R_do_slot(val, Matrix_xSym)), &info FCONE FCONE);
    UNPROTECT(1);
    return val;
}
...
{
    int n = *INTEGER(R_do_slot(x, Matrix_DimSym));
...
SEXP dtpMatrix_matrix_mm(SEXP x, SEXP y, SEXP right, SEXP trans)
{
    ...
    int *xDim = INTEGER(R_do_slot(x, Matrix_DimSym)),
	*yDim = INTEGER(R_do_slot(val, Matrix_DimSym));
    int m = yDim[0], n = yDim[1];
    int ione = 1;
    const char *uplo = uplo_P(x), *diag = diag_P(x);
    double *xx = REAL(R_do_slot(x, Matrix_xSym)),
	*vx = REAL(R_do_slot(val, Matrix_xSym));
...
SEXP dtpMatrix_matrix_solve(SEXP a, SEXP b)
{
    ...
    int *aDim = INTEGER(R_do_slot(a, Matrix_DimSym)),
	*bDim = INTEGER(R_do_slot(val, Matrix_DimSym));
    int ione = 1;
	...
	*vx = REAL(R_do_slot(val, Matrix_xSym));
	...
	/* ap = */ REAL(R_do_slot(a, Matrix_xSym)),
	/* b  = */ REAL(R_do_slot(val, Matrix_xSym)),
...
SEXP dgeMatrix_dtpMatrix_mm(SEXP x, SEXP y)
{
    ...
    int *xDim = INTEGER(R_do_slot(x, Matrix_DimSym)),
	*yDim = INTEGER(R_do_slot(y, Matrix_DimSym));
    const char *uplo = uplo_P(y), *diag = diag_P(y);
    double *yx = REAL(R_do_slot(y, Matrix_xSym)),
 	*vx = REAL(R_do_slot(val, Matrix_xSym));
