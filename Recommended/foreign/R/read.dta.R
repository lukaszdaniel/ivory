             ...
             convert.factors = c("labels","string","numeric","codes"))
{
    ...
    if(!is.data.frame(dataframe))
        stop(gettextf("'%s' argument is not an object of class %s", "dataframe", dQuote("data.frame")))
    if (version < 6L) stop("Version must be between 6 and 12")
    if (version == 9L) version <- 8L
    if (version == 11L) version <- 10L
    if (version == 12L) version <- 10L
    if (version > 12L) {
        warning("Version must be between 6 and 12: using 7")
        version <- 7L
    }
