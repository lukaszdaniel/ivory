...
function(mirrors = NULL, verbose = FALSE)
{
    ...
    check_mirror <- function(mirror) {
        ...
        if(inherits(mirror_packages, "error"))
            stop(gettextf("Reading %ssrc/contrib/PACKAGES.gz failed with message: %s",
                         mirror, conditionMessage(mirror_packages), domain = "R-tools"))
        mirror_ts1 <- tryCatch(readLines(sprintf("%s%s", mirror,
                                                 path_ts1)),
                               error = identity)
        if(inherits(mirror_ts1, "error"))
            stop(gettextf("Reading %s%s failed with message: %s",
                         mirror, path_ts1, conditionMessage(mirror_ts1), domain = "R-tools"))
        mirror_ts2 <- tryCatch(readLines(sprintf("%s%s", mirror,
                                                 path_ts2)),
                               error = identity)
        if(inherits(mirror_ts2, "error"))
            stop(gettextf("Reading %s%s failed with message: %s",
                         mirror, path_ts2, conditionMessage(mirror_ts2), domain = "R-tools"))
