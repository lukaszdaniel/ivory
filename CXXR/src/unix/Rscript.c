...
int main(int argc, char *argv[])
{
	...
	*p = '\0';
	...
	if(strcmp(argv[1], "--version") == 0) {
		...
		fprintf(stderr, "rho scripting front-end version %s.%s %s (%s-%s-%s r%d)\n",
    ...
    perror(_("rhoscript execution error"));
#else
    AppMain(ac, av);
#endif
    return res;
#else /* No execv*/
    fprintf(stderr, _("rhoscript is not supported on this system"));
    exit(1);
#endif
}
