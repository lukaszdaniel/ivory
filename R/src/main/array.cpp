...
RHIDDEN SEXP do_matrix(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (miss_nr && miss_nc) {
	if (lendat > R_INT_MAX) error(_("data is too long"));
	nr = (int) lendat;
    } else if (miss_nr) {
	if (lendat > (double) nc * R_INT_MAX) error(_("data is too long")); // incl lendat > nc == 0
	if (nc == 0) // as lendat <= nc, have lendat == 0
	    nr = 0;
	else
	    nr = (int) ceil((double) lendat / (double) nc);
    } else if (miss_nc) {
	if (lendat > (double) nr * R_INT_MAX) error(_("data is too long")); // incl lendat > nr == 0
    ...
    if ((double)nr * (double)nc > R_INT_MAX)
...
#ifndef LONG_VECTOR_SUPPORT
    if ((double)nrow * (double)ncol > R_INT_MAX)
...
#ifndef LONG_VECTOR_SUPPORT
    if ((double)nrow * (double)ncol * (double)nface > R_INT_MAX)
...
RHIDDEN SEXP do_length(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	if (length(ans) == 1 && TYPEOF(ans) == REALSXP) {
	    double d = REAL(ans)[0];
	    if (R_FINITE(d) && d >= 0. && d <= R_INT_MAX && floor(d) == d) {
                PROTECT(ans);
    ...
    if (len > R_INT_MAX) return ScalarReal((double) len);
#endif
...
#ifndef LONG_VECTOR_SUPPORT
    if (d > R_INT_MAX) error(_("too many elements specified"));
#endif
    nans = (R_xlen_t) d;
