print.survfit <- function(x, scale=1, 
 ...
 #   x <- survfit32(x)
    ...
    if (rmean != 'none') {
        ...
        if (rmean == 'individual') 
            cat(gettext("   * restricted mean with variable upper limit\n", domain = "R-survival"))
        else cat(gettextf("    * restricted mean with upper limit = %s", format(temp$end.time[1]), domain = "R-survival"), "\n", sep = "")
        }
    else print(mtemp)
...
survmean <- function(x, scale=1, rmean) {
	...
	stemp <- rep(1:nstrat,x$strata)  # the index vector for strata1, 2, etc
        ...
        last.time <- (rev(stime))[match(seq_len(nstrat), rev(stemp))]
