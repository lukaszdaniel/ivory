 ...
 */
void lqs_fitlots(double *x, double *y, int *n, int *p, int *qn, int *lts,
		int *adj, int *sample, int *nwhich, int *ntrials, double *crit,
		int *sing, int *bestone, double *bestcoef, double *pk0, double *beta) {
	int nnew = *nwhich, pp = *p;
	int i, iter, j, k, nn = *n, this, trial;
	int rank, info, n100 = 100;
	int firsttrial = 1;
	double a = 0.0, tol = 1.0e-7, sum, thiscrit, best = BIG, target, old, new,
			dummy, k0 = *pk0;
	...
	*sing = 0;
	...
	for (trial = 0; trial < *ntrials; trial++) {
		...
		F77_CALL(dqrdc2)(xr, &nnew, &nnew, &pp, &tol, &rank, qraux, pivot,
				work);
		if (rank < pp) {
			(*sing)++;
			continue;
...
/*
 ...
 subset.
 */
static int do_one(double *x, int *which, int n, int nnew, int p, double *det,
		double *d2) {
	int i, j, k;
	int rank;
...
void mve_fitlots(double *x, int *n, int *p, int *qn, int *mcd, int *sample,
