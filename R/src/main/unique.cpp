...
struct HashData
{
	...
	bool isLong;
#endif
	hlen (*hash)(SEXP, const R_xlen_t &, HashData &);
	bool (*equal)(SEXP, const R_xlen_t &, SEXP, const R_xlen_t &);
	SEXP HashTable;
...
static hlen lhash(SEXP x, const R_xlen_t &indx, HashData &d)
{
...
R_INLINE static hlen ihash(SEXP x, const R_xlen_t &indx, HashData &d)
{
...
R_INLINE static hlen rhash(SEXP x, const R_xlen_t &indx, HashData &d)
{
...
static Rcomplex unify_complex_na(const Rcomplex &z) {
    Rcomplex ans;
...
static hlen chash(SEXP x, const R_xlen_t &indx, HashData &d)
{
 ...
 * we do (upper ^ lower) */
inline static hlen cshash(SEXP x, const R_xlen_t &indx, HashData &d)
{
...
inline static hlen shash(SEXP x, const R_xlen_t &indx, HashData &d)
{
    unsigned int k;
    const char *p;
...
static bool lequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
...
inline static bool iequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
    if (i < 0 || j < 0) return false;
...
inline static bool requal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
    if (i < 0 || j < 0) return false;
...
static int cplx_eq(const Rcomplex &x, const Rcomplex &y)
{
...
static bool cequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
    if (i < 0 || j < 0) return false;
    return cplx_eq(COMPLEX_ELT(x, i), COMPLEX_ELT(y, j));
}

inline static bool sequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
	if (i < 0 || j < 0)
		return false;
...
static bool rawequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
    if (i < 0 || j < 0) return false;
    return (RAW_ELT(x, i) == RAW_ELT(y, j));
}

static hlen vhash(SEXP x, const R_xlen_t &indx, HashData &d)
{
    int i;
    unsigned int key;
...
static bool vequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
    if (i < 0 || j < 0) return false;
...
*/
static void MKsetup(R_xlen_t n, HashData &d, const R_xlen_t &nmax)
{
#ifdef LONG_VECTOR_SUPPORT
...
static void HashTableSetup(SEXP x, HashData &d, const R_xlen_t &nmax)
{
    d.useUTF8 = false;
    d.useCache = true;
    switch (TYPEOF(x)) {
...
static int isDuplicated(SEXP x, const R_xlen_t &indx, HashData &d)
{
#ifdef LONG_VECTOR_SUPPORT
    if (d.isLong) {
...
static void removeEntry(SEXP table, SEXP x, const R_xlen_t &indx, HashData &d)
{
#ifdef LONG_VECTOR_SUPPORT
    if (d.isLong) {
	double *h = HTDATA_DBL(d);
...
R_xlen_t sorted_real_count_NANs(SEXP x) {
	...
	error(_("sorted_real_count_NANs got unsorted vector: this should not happen"));
...
#define FIND_NAN_EDGE(yes, no, final)          \
	do                                         \
	{                                          \
...
#define SORTED_DUP_NANS(itype, istart, icond, iter)                          \
	do                                                                       \
	{                                                                        \
	...
	nas1st = Rboolean(KNOWN_NA_1ST(REAL_IS_SORTED(x)));
...
#define SORTED_ANYDUP_NONNANS_FROM_LAST(start, count, tmpvar, eetype, vvtype)    \
	do                                                                           \
	{                                                                            \
...
#define SORTED_ANYDUP_NONNANS_FROM_FIRST(start, count, tmpvar, eetype, vvtype) \
	do                                                                         \
	{                                                                          \
		tmpvar = vvtype##_ELT(x, start);                                       \
		ITERATE_BY_REGION_PARTIAL(x, xptr, idx, nb, eetype,                    \
...
#define SORTED_ANYDUP_NANS(start, count, itype, istart, icond, iter)         \
	do                                                                       \
	{                                                                        \
	...
	sorted = INTEGER_IS_SORTED(x);
	if(!KNOWN_SORTED(sorted))
	    error(_("sorted_any_duplicated got unsorted vector: this should not happen"));
	if(from_last) {
	...
	na1st = Rboolean(KNOWN_NA_1ST(sorted));
	...
	if(from_last) {
		...
		SORTED_ANYDUP_NANS(napivot, numnas, PARTIAL_REV, nb - 1, i >= 0, i--);
		SORTED_ANYDUP_NONNANS_FROM_LAST(0, napivot, rtmp, double, REAL);
	    }
	} else { // !from_last
	    if(na1st) {
		SORTED_ANYDUP_NANS(0, numnas, PARTIAL, 0, i < nb, i++);
		SORTED_ANYDUP_NONNANS_FROM_FIRST(numnas, napivot, rtmp, double, REAL);
	    } else {
		SORTED_ANYDUP_NONNANS_FROM_FIRST(0, napivot, rtmp, double, REAL);
...
*/
RHIDDEN SEXP do_duplicated(SEXP call, SEXP op, SEXP args, SEXP env)
{
    SEXP x, dup, ans;
    R_xlen_t i, k, n;
	...
	if(PRIMVAL(op) == 2) {
	    ...
	    if(ind > R_INT_MAX) return ScalarReal((double) ind);
...
RHIDDEN SEXP do_match(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
RHIDDEN SEXP do_pmatch(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
RHIDDEN SEXP do_charmatch(SEXP call, SEXP op, SEXP args, SEXP env)
{
    SEXP ans, input, target;
    const char *ss, *st;
    bool useBytes = false, useUTF8 = false;
...
RHIDDEN SEXP do_matchcall(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
static SEXP rowsum(SEXP x, SEXP g, SEXP uniqueg, SEXP snarm, SEXP rn)
{
	...
	for(int i = 0; i < p; i++) {
		...
		if (xjpo == NA_INTEGER) {
		    ...
		    double dtmp = itmp;
		    dtmp += xjpo;
		    if (dtmp < INT_MIN || dtmp > R_INT_MAX) itmp = NA_INTEGER;
		    else itmp += xjpo;
...
RHIDDEN SEXP do_rowsum(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
    if(PRIMVAL(op) == 1)
...
RHIDDEN SEXP do_makeunique(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
static bool csequal(SEXP x, const R_xlen_t &i, SEXP y, const R_xlen_t &j)
{
...
RHIDDEN SEXP do_sample2(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
    SEXP ans;
