...
static void RemakeNextSEXP(FILE *fp, NodeInfo *node, int version, InputRoutines *m, SaveLoadData *d)
{
    ...
    case PROMSXP:
    case ENVSXP:
	Rf_error(_("Loading pre-version-1 serialization not (yet) supported in rho"));
			 ...
			 InputRoutines *m, SaveLoadData *d)
{
	...
	PROTECT(s = pos ? VECTOR_ELT(sym_table, pos - 1) : nullptr);
	break;
    case ENVSXP:
	pos = m->InInteger(fp, d);
	...
	break;
    case WEAKREFSXP:
	PROTECT(s = R_MakeWeakRef(nullptr, nullptr, nullptr, FALSE));
	break;
    case SPECIALSXP:
    case BUILTINSXP:
	R_AllocStringBuffer(MAXELTSIZE - 1, &(d->buffer));
...
SEXP attribute_hidden do_save(/*const*/ rho::Expression* call, const rho::BuiltInFunction* op, rho::RObject* list_, rho::RObject* file_, rho::RObject* ascii_, rho::RObject* version_, rho::RObject* envir_, rho::RObject* eval_promises_)
{
	...
	R_SaveToFileV(s, fp, INTEGER(ascii_)[0], version);
    }
    catch (...) {
	fclose(fp);
	throw;
