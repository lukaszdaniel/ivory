             ...
             binary = FALSE, ..., av2 = NULL) ## ... is passed to installed.packages().
{
    ...
    if(sum(m0) && !is.null(av2)) {
        ...
        if(nrow(av2)) {
            ...
            msg2 <- gettextf("Package %s version %s is in the repositories but depends on %s", sQuote(av2[, "Package"]), av2[, "Version"], unlist(x))
        }
    }
    if(sum(m0)) {
        msg <- paste0(if(binary) "as a binary package ", "for this version of R")
